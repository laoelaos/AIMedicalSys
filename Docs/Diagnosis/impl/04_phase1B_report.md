# 诊断报告：OOD Phase1_B 实现审查问题分析（v7.1）
...
| P1 | T3 | 真实缺陷 | 实现编码偏差 | GlobalExceptionHandler / GlobalErrorCode | - |
| P1 | T6 | 真实缺陷 | 实现编码偏差 | AuthController | - |
| P1 | T7 | 真实缺陷 | 实现编码偏差 | AuthController | - |
| P1 | T8 | 真实缺陷 | 实现编码偏差 | AuthService / AuthServiceImpl | - |
...
| P2 | T9 | 真实缺陷 | 实现编码偏差 | UserConverter / UserFacadeImpl | - |
...
**统计：** 26/26 项确认为真实缺陷（0 项误报）。其中 21 项根因归属于纯实现编码偏差，1 项归属于设计与实现约定不一致（T14），4 项归属于测试覆盖不完整（T15/T16/T17/T18），1 项归属于 Spring 配置冲突（T26）。

> **变更说明（v7 → v7.1）：** 诊断报告 v7 阶段识别出 T3/T8/T9/T22 共 4 项存在 OOD 设计粒度不足的根因贡献。在 v7.1 阶段，对 OOD 文档（`Docs/05_ood_phase1_B.md`）进行了如下补全：
> - T3 → 新增 10.3 节"错误消息模板插值管线"，明确 `BusinessException args → GlobalExceptionHandler → Result` 的完整插值链路
> - T8 → 新增 1.3 节 `SecurityAuditLogger` 抽象 + 4.8 节"安全审计日志设计" + 2.1 节目录结构中的 `audit/` 包
> - T9 → 新增 1.3 节 `UserConverter` 抽象 + 5.2 节"User → UserInfoResponse 转换的唯一来源"职责约束
> - T22 → 补全 5.1 节 `PermissionFunction` 实体变更表中 `component` 字段映射条目
>
> 上述 OOD 补全后，4 项缺陷的根因完整归属于实现侧，OOD 文档提供了可直接遵循的设计契约。T3/T8/T9/T22 从"OOD 维度粒度不足"分类迁移至"OOD 维度充分"分类。
...
| **T3** | 10.2 节 + 10.3 节 | OOD 10.2 定义了 ACCOUNT_LOCKED 的模板消息 `"账户已锁定，请{锁定时间}后重试"`，10.3 节明确设计 `BusinessException args → GlobalExceptionHandler → Result` 的完整插值管线（插值时机、插值方法、Result 响应格式）。实现者未实现 `GlobalExceptionHandler` 的模板插值逻辑，导致客户端收到模板原文。 |
...
| **T8** | 1.3 节 + 3.1.4 节 + 4.8 节 | OOD 1.3 定义了 `SecurityAuditLogger` 抽象、3.1.4 节登出流程步骤 4 明确要求记录安全审计日志、4.8 节详细定义审计事件字段、日志格式、Phase 1 日志文件实现及 Phase 2 数据库迁移路径。实现者未记录任何审计日志。 |
| **T9** | 1.3 节 + 5.2 节 | OOD 1.3 定义 `UserConverter` 为 `User` 与 `UserInfoResponse` 之间转换的唯一来源，5.2 节"User → UserInfoResponse 转换的唯一来源"明确约束 `UserFacadeImpl.toUserInfoResponse()` 必须委托 `UserConverter`。实现者在两处独立维护重复转换逻辑，且 `UserConverter` 缺少关键过滤逻辑。 |
...
| **T22** | 5.1 节 + 5.2 节 | OOD 5.1 节 PermissionFunction 实体变更表已补充 `component` 字段映射条目（`@Column(name = "component") private String component;`），5.2 节 MenuResponse 已定义 component 字段。实现者未将实体字段与表列映射。 |
...
经 v7.1 阶段 OOD 文档补全后，26 项缺陷中 0 项的根因涉及 OOD 设计粒度不足。T3/T8/T9/T22 已在 v7 阶段识别为"OOD 维度粒度不足"，通过以下 OOD 文档补全迁移至"OOD 维度充分"分类：

- **T3** → OOD 10.3 节"错误消息模板插值管线"补全了 `BusinessException args → GlobalExceptionHandler → Result` 的完整插值管线设计（插值时机、插值方法、Result 响应格式、扩展性约束）
- **T8** → OOD 1.3 节新增 `SecurityAuditLogger` 抽象、3.1.4 节登出流程步骤 4 明确审计日志记录要求、4.8 节"安全审计日志设计"详细定义接口、字段、Phase 1 文件实现和 Phase 2 数据库迁移路径
- **T9** → OOD 1.3 节明确 `UserConverter` 为 `User → UserInfoResponse` 转换的唯一来源，5.2 节"User → UserInfoResponse 转换的唯一来源"显式约束 `UserFacadeImpl.toUserInfoResponse()` 必须委托 `UserConverter`
- **T22** → OOD 5.1 节 PermissionFunction 实体变更表补加 `component` 字段映射条目（`@Column(name = "component") private String component;`）
...
| **批次 6：基础设施修复** | T8, T9, T11, T22, T23 | T8 按 OOD 4.8 节设计实现 SecurityAuditLogger（日志文件实现）+ SecurityAuditEvent + LoggingSecurityAuditLogger；T9 按 OOD 1.3 + 5.2 节"User → UserInfoResponse 转换的唯一来源"先修复 UserConverter 三处关键过滤逻辑（`Role::getEnabled` 过滤、`Comparator.nullsLast(Comparator.naturalOrder())`、`PermissionFunction::getEnabled` 过滤），再让 UserFacadeImpl 委托 UserConverter；T11 需新增异常类型（AccountDisabledAuthenticationException）；T22 按 OOD 5.1 节补全的 component 字段映射新增 JPA 实体字段，需评估 schema 变更影响；T23 需扩展 Repository 查询方法。**T22 从批次 2 移入本批次**，因其涉及 JPA 实体字段新增，非纯文本修改。**注意 T8 与 T9 的隐含交叉**：T8 新建的 SecurityAuditLogger 若被 UserFacadeImpl 或 AuthService 引用记录用户转换操作的审计日志，T9 的 UserConverter 合并后需同步引入日志记录。批次排期时应将 T8/T9 分配给同一位开发者或至少互相同步接口设计。 |
...
**批次 6 隐含依赖声明（新增）：** T8 和 T9 在表面无直接 API 依赖，但存在日志逻辑交叉的可能场景：若 UserFacadeImpl.toUserInfoResponse() 或 UserConverter.toUserInfoResponse() 需要记录转换操作的审计日志（如管理员查询用户信息时的审计需求），T8 的 SecurityAuditLogger 接口将成为 T9 的依赖。建议批次 6 内部排期：T8 先完成（按 OOD 4.8 节确定 SecurityAuditLogger 接口、SecurityAuditEvent 字段、LoggingSecurityAuditLogger 实现），T9 随后（必要时引入日志记录）。T22 和 T23 与 T8/T9 无交叉依赖。

> **v7.1 变更：** T8 和 T9 的 OOD 设计契约已在 v7.1 阶段补全（T8 → OOD 4.8 节安全审计日志设计；T9 → OOD 1.3 + 5.2 节"User → UserInfoResponse 转换的唯一来源"），实现者可按 OOD 直接落地，无需自行设计接口/职责划分。T22 的 OOD 实体变更表条目也已在 v7.1 补全（T22 → OOD 5.1 节 PermissionFunction component 字段映射）。
...
- **判定：** 真实缺陷
- **根因：** 实现编码偏差（OOD 充分：10.2 节 + 10.3 节完整定义了模板消息与插值管线）
- **分析：**
  ...
  - **因果链：** OOD 10.2 定义了 ACCOUNT_LOCKED 的动态消息机制（IP 锁定 30 分钟 vs 用户名锁定 15 分钟），10.3 节明确设计了 `BusinessException args → GlobalExceptionHandler → Result` 的完整插值管线（插值时机、插值方法、Result 响应格式），实现者未在 `GlobalExceptionHandler` 中实现模板插值逻辑。
- **修改建议：**
  - 代码位置：
  ...
  - 修改方式：按 OOD 10.3 节设计，在 `GlobalExceptionHandler` 中完成插值——因为它是消息管线的统一出口。实现一个 `formatMessage(String template, Object[] args)` 方法，优先使用 `MessageFormat.format(template, args)`，对 `{锁定时间}` 等命名占位符降级为 `String.replace`。
  - 涉及的相关文件：`GlobalExceptionHandler.java`、`Result.java`（可能需要新增重载方法）
  - 验证方式：编写 `GlobalExceptionHandlerTest`，Mock 带有 args 的 BusinessException，验证响应中的 message 包含插值后的内容（如 `"账户已锁定，请30分钟后重试"`）而非模板原文。
- **风险与注意事项：**
  ...
  - 按 OOD 10.3 节"扩展性约束"设计，当前插值逻辑仅处理 ACCOUNT_LOCKED 这一个模板消息（`{锁定时间}` 占位符）。若后续增加更多模板消息，需确保 `formatMessage` 方法可扩展。建议使用 `MessageFormat` 或简单的 `String.replace`。
...
- **判定：** 真实缺陷
- **根因：** 实现编码偏差（OOD 充分：1.3 节定义 `SecurityAuditLogger` 抽象、3.1.4 节步骤 4 明确审计日志记录要求、4.8 节详细定义审计日志设计）
- **分析：**
  - `AuthService` 接口（`service/AuthService.java:13`）定义 `void logout(String token)`，未接收 refreshToken 参数。
  - `AuthServiceImpl.logout()`（`service/impl/AuthServiceImpl.java:147-164`）同样只接受 token，未处理 refreshToken，未记录任何审计日志。
  - `AuthController.logout()`（`controller/AuthController.java:43-52`）虽接收了 `refreshTokenRequest` 参数，但未将其传递给 `authService.logout()`。
  - OOD 3.1.4 步骤 1 和 4.4 规定：登出请求可选携带 refreshToken 字段，Phase 1 应记录安全日志。OOD 1.3 定义 `SecurityAuditLogger` 接口、3.1.4 步骤 4 明确要求调用 `SecurityAuditLogger.logAudit(...)` 记录登出事件，4.8 节详细定义了审计事件字段（`eventType=LOGOUT`、`userId`、`username`、`clientIp`、`refreshTokenProvided`、`refreshTokenMasked`、`success`、`timestamp`）、Phase 1 日志文件实现（`LoggingSecurityAuditLogger`）及 Phase 2 数据库迁移路径。
- **修改建议：**
  - **默认行动路径（推荐）：** 按 OOD 1.3 + 3.1.4 + 4.8 节设计，在 `common-module-impl/auth/audit/` 包下新增 `SecurityAuditLogger` 接口、`SecurityAuditEvent` 值对象、`LoggingSecurityAuditLogger` 实现类（写入 `${LOG_DIR}/audit/security-audit.log`）。`AuthServiceImpl.logout()` 接收 refreshToken 参数后，通过 `SecurityAuditLogger.logAudit(...)` 记录登出审计事件。
  - **备选方案：** 若项目已有 `AuditLogger`、`AuditAspect`、`@AuditLog` 或 `AuditEventPublisher` 等现成审计框架（在 `AIMedical/backend/` 下搜索 `class.*Audit` 或检查 `common-module`/`framework` 模块），可新增 `AdapterSecurityAuditLogger` 包装现有框架，遵循 OOD 4.8 节定义的事件字段规范。
  - 代码位置：
    - `auth/audit/SecurityAuditLogger.java`（新建）：接口定义 `void logAudit(SecurityAuditEvent event)`
    - `auth/audit/SecurityAuditEvent.java`（新建）：值对象，字段见 OOD 4.8 节
    - `auth/audit/LoggingSecurityAuditLogger.java`（新建）：Phase 1 日志文件实现
    - `service/AuthService.java:13`：接口签名改为 `void logout(String token, String refreshToken)`
    - `service/impl/AuthServiceImpl.java:147-164`：接收 refreshToken 参数，构造 `SecurityAuditEvent` 并调用 `SecurityAuditLogger.logAudit(...)`
  ...
  - 涉及的相关文件：`AuthService.java`、`AuthServiceImpl.java`、`AuthController.java`、新建 `auth/audit/` 包下三个类
  - 验证方式：编写 `AuthControllerTest` 验证 logout 请求携带 refreshToken 时，AuthService.logout 被调用时 refreshToken 参数非 null；编写 `LoggingSecurityAuditLoggerTest` 验证登出事件以结构化格式写入审计日志文件。
- **风险与注意事项：**
  - 接口签名变更影响所有 AuthService 的实现者和调用者。目前仅 AuthServiceImpl 一个实现类，影响范围可控。若后续有 mock 测试，需同步更新 mock 调用。
  - 审计日志写入失败不应阻塞主业务流，需捕获 `IOException` 并降级记录到业务日志（`log.warn("Audit log write failed: {}", e.getMessage())`）。

### T9: UserConverter 与 UserFacadeImpl 存在重复且不一致的转换逻辑（P2）

- **判定：** 真实缺陷
- **根因：** 实现编码偏差（OOD 充分：1.3 节定义 `UserConverter` 为 `User → UserInfoResponse` 转换的唯一来源，5.2 节"User → UserInfoResponse 转换的唯一来源"明确约束 `UserFacadeImpl.toUserInfoResponse()` 必须委托 `UserConverter`）
- **分析：**
  ...
  - OOD 1.3 抽象一览中明确 `UserConverter` 为 `@Component class`，承担角色解析（按 `Role::getEnabled` 过滤 + 按 `Role::getSort` 排序 + `Comparator.nullsLast(Comparator.naturalOrder())` 防止 NPE）、权限解析（按 `PermissionFunction::getEnabled` 过滤）、`User → UserInfoResponse` 字段映射三大职责，并显式约束 `UserFacadeImpl.toUserInfoResponse()` **必须委托** `UserConverter`。
  - OOD 5.2 节"User → UserInfoResponse 转换的唯一来源"重申该职责划分。
- **修改建议：**
  ...
  - 修改方式：
    ...
    - **推荐方案：以 OOD 1.3 定义的 UserConverter 规范为正确基准**，将 UserConverter 的三处不一致修正（补充 `Role::getEnabled` 过滤、改用 `Comparator.nullsLast(Comparator.naturalOrder())`、补充 `PermissionFunction::getEnabled` 过滤），然后 `UserFacadeImpl` 注入 `UserConverter` 并委托 `userConverter.toUserInfoResponse(user)`，删除 `UserFacadeImpl` 中的私有转换方法。修复后的 UserConverter 满足 OOD 1.3 + 5.2 节定义的转换职责。
    - 或方案 B：直接废弃 UserConverter，将 `toUserInfoResponse()` 及三个辅助方法统一保留在 UserFacadeImpl 中，删除 UserConverter。**但此方案违反 OOD 1.3 + 5.2 节"UserConverter 为转换唯一来源"的职责约束，不推荐**。
  - 涉及的相关文件：`UserConverter.java`、`UserFacadeImpl.java`
  - 验证方式：对 `UserConverter` 补充测试，覆盖已禁用角色、已禁用权限、Role.sort 为 null 的场景，验证转换结果与 `UserFacadeImpl` 一致；验证 `UserFacadeImpl` 委托 `UserConverter` 后输出结果不变。
- **风险与注意事项：**
  ...
  - **OOD 维度分析（补充）：** OOD 1.3 抽象一览和 5.2 节"User → UserInfoResponse 转换的唯一来源"已显式定义 `UserFacadeImpl.toUserInfoResponse()` 应委托 `UserConverter`，实现者本可在一处完成转换逻辑，避免两处独立维护。当前实现者在两处独立维护重复转换逻辑，且 `UserConverter` 自身存在三处关键过滤逻辑缺失，是实现侧的偏差。
...
- **风险与注意事项：**
  - 测试中 mock 的 PermissionFunction 实例需正确设置 id、parentId、sortOrder 等字段。注意 `convertToMenuResponse` 中 `component` 字段在 T22 修复前硬编码为 null，测试断言中 component 应预期为 null；T22 修复后断言应基于 `function.getComponent()` 的返回值。
...
- **判定：** 真实缺陷
- **根因：** 实现编码偏差（OOD 充分：5.1 节实体变更表已补全 `component` 字段映射条目 + 5.2 节 MenuResponse/MenuUpdateRequest 已定义 component 字段）
- **分析：**
  ...
  - OOD 5.1 节 `Function.java → PermissionFunction.java` 实体变更表已补充 `component` 字段映射条目：`@Column(name = "component") private String component;`，添加 getter/setter；schema.sql 已存在 `component VARCHAR(...)` 列，无需 DDL 变更。OOD 5.2 节 MenuResponse 和 MenuUpdateRequest 已定义 component 字段，与 5.1 节实体变更一致。
- **修改建议：**
  - 代码位置：
    - `permission/PermissionFunction.java`：按 OOD 5.1 节补全后的规范新增 `private String component;` 字段，添加 `@Column(name = "component")` 注解及 getter/setter
...
- **风险与注意事项：**
  ...
  - 新增字段后需添加 getter/setter 方法。`PermissionFunction` 当前未使用 Lombok，需手动添加。推荐显式添加 `@Column(name = "component")` 以提高代码可读性并避免 Hibernate 元模型中的潜在歧义（与 OOD 5.1 节补全条目一致）。
