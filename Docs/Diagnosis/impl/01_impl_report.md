...
- **2 项为真实代码缺陷**（问题8、问题10）
- **1 项为 OOD 文档问题**（问题1）— ✅ 已修复（2026-06-17，详见问题1 状态标记）
- **7 项为其他类型问题**（问题2/3/4/5/6/7/9，包括设计与代码偏离、版本偏离、测试覆盖不足等）— 其中问题4 OOD 侧已修复（2026-06-17，详见问题4 状态标记）
...
**状态：✅ 已完成（2026-06-17）**
- `Docs/04_ood_phase0.md` §1.3 核心抽象一览表新增 `BaseEnum` 行（`BaseEntity` 与 `GlobalExceptionHandler` 之间）
- `Docs/04_ood_phase0.md` §3.2 数据实体基类新增 `BaseEnum` 完整定义段落（职责、方法签名、接口定义、协作对象、与 `ErrorCode` 的边界、为何使用 interface）
- 代码侧未修改
...
**用户意见：** 降 OOD 到 3.2.5（推荐）。同步更新 OOD §2.1 父 POM 骨架（`04_ood_phase0.md:145` 3.3.0 → 3.2.5）和 §1.4 运行时环境表。保持代码侧 `pom.xml` 不变。

**状态：✅ 已完成（2026-06-17）**
- `Docs/04_ood_phase0.md` §1.4 运行时环境表：`Spring Boot 3.3.0 最低要求` → `Spring Boot 3.2.5 最低要求`
- `Docs/04_ood_phase0.md` §2.1 父 POM 骨架（line 146）：`<version>3.3.0</version>` → `<version>3.2.5</version>`
- `Docs/04_ood_phase0.md` §2.1 springdoc-openapi 依赖注释：3.3.x 未验证提示 → 3.2.5 已验证提示，并将 3.3.x 降为未来升级风险提示
- `Docs/04_ood_phase0.md` §8.3 选型说明：springdoc-openapi 兼容性描述同步从"3.3.x 未验证"改为"3.2.5 已验证"
- 代码侧 `backend/pom.xml` 未修改（保持 3.2.5）
...
**用户意见：** 代码向设计对齐（推荐）。向 `backend/pom.xml` 的 `<dependencyManagement>` 段补充 5 个 starter 条目，版本由 Spring Boot parent BOM 兜底管理。
...
**用户意见：** 从 `backend/common/pom.xml` 移除 `spring-boot-starter-validation` 依赖（推荐）。移除后运行 `mvn compile -pl common` 验证编译通过。子模块（patient/doctor/admin）已独立声明 validation starter，移除不影响其编译。
...
**用户意见：** 将 `patient`、`doctor`、`admin` 三个 `<ignoredUnusedDeclaredDependency>` 条目从父 `backend/pom.xml` 移入 `backend/application/pom.xml` 自身的 `maven-dependency-plugin` 配置中（推荐）。注意 `application/pom.xml` 当前不含该插件声明，需从头编写完整插件声明（使用父 POM `pluginManagement` 中已声明的版本，无需指定 `version`）。完成后在 application 模块上运行 `mvn dependency:analyze` 验证门禁通过。
...
**用户意见：** 现在修复（推荐）。在 `GlobalExceptionHandler.java` 中补充 `HttpMessageNotReadableException`（→400 `PARAM_INVALID` + `log.warn`）和 `HttpMessageNotWritableException`（→500 `SYSTEM_ERROR` + `log.error`）两个 `@ExceptionHandler` 方法。同步在 `GlobalExceptionHandlerTest` 中新增单元测试，通过直接调用 handler 方法传入异常实例验证响应格式和日志级别（Phase 0 测试策略）。
...
**用户意见：** 补充 ListAppender 日志断言（推荐）。采用"直接方案"——对当前代码行为（首次调用 → ERROR、后续 → WARN）编写日志断言，在断言旁显式注释 K3 依赖风险；同时在 `known_issues.md` K3 条目中同步记录测试方法位置和依赖关系。新增测试方法 `shouldLogErrorOnFirstCallThenWarnOnSubsequent`，通过 public 方法路径（`triage()`）触发 `handleEmptyDelegates` 验证。
...
**用户意见：** 现在修复（推荐）。修改 `packages/shared/src/api/index.ts` 错误拦截器实现统一格式转换（NETWORK_ERROR/UNAUTHORIZED/FORBIDDEN/HTTP_ERROR），所有错误分支统一使用 `Promise.resolve()`。**TypeScript 类型方案：方案A discriminated union**——在 `types/index.ts` 中新增 `ApiSuccess<T>`、`ApiError`、`ApiResponse<T>` 三类型，成功拦截器统一返回 `ApiResponse<T>`（将现有 `return response.data.data` 改为 `return response.data` 与类型对齐），消费方通过 `if (res.code === 'SUCCESS')` 判别式自动缩小类型。
...
| 问题类型 | 包含问题 | 数量 | 修复状态 |
|---------|---------|------|---------|
| **① 真实代码缺陷**（设计要求的功能未实现或实现错误） | 问题8（GlobalExceptionHandler 缺少序列化异常处理器）、问题10（ApiClient 错误拦截器未实现 NETWORK_ERROR） | **2 项** | 待修复 |
| **② 误报**（审查报告提出的问题实际不存在） | 无 | **0 项** | — |
| **③ OOD 文档问题**（设计文档存在矛盾、偏差、不完善或错误） | 问题1（BaseEnum 未在 §1.3 定义） | **1 项** | ✅ 已修复（2026-06-17） |
| **④ 其他类型问题**（不属于以上三类的问题） | 问题2/3（目录结构与设计偏离）、问题4（版本偏离）、问题5/6/7（POM 配置偏离设计规约）、问题9（测试覆盖不充分） | **7 项** | 部分待修复（问题4 OOD 侧已修复） |
