...
### 1.2 权限模型要求
- **三级权限模型**：角色 → 岗位 → 功能（Role → Post → Function）
- 用户通过岗位关联功能权限
- 前端按权限动态渲染菜单与按钮
...
| 接口路径 | HTTP方法 | 所属Controller | 功能描述 | 权限要求 |
|----------|----------|----------------|----------|----------|
| `/api/menu/tree` | GET | MenuController | 获取当前用户菜单树 | 已认证用户 |
| `/api/menu/all` | GET | MenuController | 获取所有菜单 | ADMIN |
| `/api/menu/{id}` | GET | MenuController | 获取菜单详情 | ADMIN |
| `/api/menu` | POST | MenuController | 创建菜单 | ADMIN |
| `/api/menu/{id}` | PUT | MenuController | 更新菜单 | ADMIN |
| `/api/menu/{id}` | DELETE | MenuController | 删除菜单 | ADMIN |
...
```json
{
  ...
  "parentId": "number (选填，父菜单ID)",
  "path": "string (选填，路由路径)",
  "icon": "string (选填，图标名称)",
  "type": "string (选填，类型：MENU/BUTTON/API)",
  "sortOrder": "number (选填，排序号，默认0)",
  "visible": "boolean (选填，是否可见，默认true)",
  "enabled": "boolean (选填，是否启用，默认true)"
}
```
...
> **架构说明**：实际实现采用四实体模型，与原设计的三实体模型存在差异。
...
| nickname | VARCHAR(50) | NOT NULL | 昵称（对应设计的realName） |
| phone | VARCHAR(20) | — | 手机号 |
| email | VARCHAR(100) | — | 邮箱 |
| enabled | TINYINT | DEFAULT 1 | 是否启用(1启用/0禁用) |
| user_type | VARCHAR(20) | NOT NULL | 用户类型(ADMIN/DOCTOR/PATIENT) |
| deleted | TINYINT | DEFAULT 0 | 是否删除(1删除/0未删除) |
...
| role_id | BIGINT | FOREIGN KEY | 所属角色ID |
| enabled | TINYINT | DEFAULT 1 | 是否启用 |
| deleted | TINYINT | DEFAULT 0 | 是否删除 |
| created_at | DATETIME | DEFAULT CURRENT_TIMESTAMP | 创建时间 |
| updated_at | DATETIME | DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP | 更新时间 |

### 6.4 功能表 (`sys_function`)

| 字段名 | 类型 | 约束 | 说明 |
|--------|------|------|------|
| id | BIGINT | PRIMARY KEY, AUTO_INCREMENT | 功能ID |
| code | VARCHAR(100) | UNIQUE, NOT NULL | 功能编码 |
| name | VARCHAR(50) | NOT NULL | 功能名称 |
| description | VARCHAR(255) | — | 功能描述 |
| parent_id | BIGINT | FOREIGN KEY | 父功能ID |
...
### 6.6 用户岗位关联表 (`user_post`)

| 字段名 | 类型 | 约束 | 说明 |
|--------|------|------|------|
| user_id | BIGINT | FOREIGN KEY | 用户ID |
| post_id | BIGINT | FOREIGN KEY | 岗位ID |

### 6.7 岗位功能关联表 (`post_function`)

| 字段名 | 类型 | 约束 | 说明 |
|--------|------|------|------|
| post_id | BIGINT | FOREIGN KEY | 岗位ID |
| function_id | BIGINT | FOREIGN KEY | 功能ID |
