...
| `UserConverter` | @Component class | `User` 实体与 `UserInfoResponse` DTO 之间的转换器，作为转换逻辑的唯一来源；承担以下职责：(a) 角色解析（按 `Role::getEnabled` 过滤已禁用角色，按 `Role::getSort` 升序取首个，`Comparator.nullsLast(Comparator.naturalOrder())` 防止 sort 为 null 时 NPE）；(b) 权限解析（按 `PermissionFunction::getEnabled` 过滤已禁用权限）；(c) `User` → `UserInfoResponse` 字段映射。`UserFacadeImpl.toUserInfoResponse()` **必须委托** `UserConverter` 完成转换，禁止在两处独立维护重复逻辑 |
| `SecurityAuditLogger` | interface | 安全审计日志记录器，封装登录/登出/密码变更/Token 刷新等安全敏感操作的审计事件记录。提供统一方法签名 `void logAudit(SecurityAuditEvent event)`，由 `AuthServiceImpl` 等业务层调用。Phase 1 采用日志文件实现（`LoggingSecurityAuditLogger`），Phase 2 可平滑迁移至数据库审计表实现而不影响调用方。详细设计见 4.8 节 |
...
└── common-module-impl/src/main/java/com/aimedical/modules/commonmodule/
    ...
    │   ├── audit/
    │   │   ├── SecurityAuditLogger.java         # 安全审计日志接口
    │   │   ├── SecurityAuditEvent.java          # 审计事件值对象
    │   │   └── LoggingSecurityAuditLogger.java  # 日志文件实现（Phase 1）
    │   ├── blacklist/
...
**ACCOUNT_LOCKED 消息插值机制**（实现细节见 10.3 节）：上述两条消息的"30 分钟"和"15 分钟"由运行时动态生成，依赖消息模板插值管线。`AuthServiceImpl` 检测到锁定后，构造 `BusinessException(GlobalErrorCode.ACCOUNT_LOCKED, "30分钟")` 或 `"15分钟"`，第二个参数作为 args 传入。`GlobalExceptionHandler.handleBusinessException()` 在统一出口完成模板插值（将 `ErrorCode.getMessage()` 中的 `{锁定时间}` 占位符替换为 args[0]），最终响应体 message 字段返回插值后的完整文本（如 `"账户已锁定，请30分钟后重试"`）
...
**Token 刷新审计**：步骤 6（用户名锁定拒绝刷新）和步骤 7（用户禁用/删除拒绝刷新）属于安全敏感操作，必须记录审计日志。审计事件 `eventType=TOKEN_REFRESH_REJECTED` 至少包含：`userId`、`username`、`clientIp`、`reason`（LOCKED / DISABLED / DELETED）、`timestamp`。步骤 10（签发新 Token）成功完成后，记录 `eventType=TOKEN_REFRESH_SUCCESS`，包含 `userId`、`username`、`clientIp`、`newJti`（Access Token jti）、`timestamp`。审计日志通过 `SecurityAuditLogger.logAudit(...)` 统一输出
...
4. **安全审计日志记录**：通过 `SecurityAuditLogger.logAudit(...)` 记录登出事件，无论是否携带 `refreshToken` 均记录。审计事件字段详见 4.8 节，至少包含：`eventType=LOGOUT`、`userId`（从 Access Token 解析）、`username`、`clientIp`、`refreshTokenProvided`（boolean，标识请求体是否携带 refreshToken）、`refreshTokenMasked`（若携带则记录掩码后的 refreshToken 前 8 位 + "***"）、`success=true`、`timestamp`。审计日志独立于业务日志，存储于专用审计文件或后续迁移至数据库审计表
5. 返回成功响应
6. 前端 finally 块清除 localStorage 中的 token 和 user 信息。即使后端登出 API 因 token 过期/无效返回 401，前端仍应在 finally 块中清除本地认证数据。Token 黑名单的登出记录是"尽力而为"的最优努力，不应阻止本地登出

**职责归属**：`AuthController.logout()` 负责从 SecurityContext 获取当前用户标识（userId、username），构造审计事件并委托 `AuthService.logout(...)` 记录；`AuthService.logout(...)` 负责实际的 TokenBlacklist 写入和审计日志调用。Controller 不直接调用 `SecurityAuditLogger`，统一在 Service 层封装审计语义，确保审计策略的演进（文件→数据库）不渗透到 Controller 层
...
实现方式：`InMemoryRateLimitGuard` 使用 `ConcurrentHashMap<String, SlidingWindowCounter>`，键为 IP 地址，值为滑动窗口计数器。每个 key 的 Deque 在 `ConcurrentHashMap.compute` 闭包内原子访问，无需额外锁。限流检查仅发生在登录请求，并发量低，锁竞争可忽略。
...
安全审计日志记录安全敏感操作（登录失败/成功、登出、Token 刷新、密码变更、用户禁用/启用等），用于事后追溯、安全审计、合规检查。审计日志与业务日志分离，独立存储，满足以下要求：

- **不可篡改**：审计日志以 append-only 方式写入，文件实现使用 append 模式，禁用覆盖
- **可追溯**：每条审计事件包含操作人 userId、操作时间戳（毫秒精度 ISO-8601）、操作类型、客户端 IP、操作结果
- **结构化**：审计事件采用结构化字段（key=value），便于日志采集系统解析；Phase 1 使用日志文件，Phase 2 可迁移至数据库审计表
- **失败容忍**：审计日志写入失败不应阻塞主业务流，但应在业务日志中记录告警
...
**SecurityAuditEvent 值对象字段**：

| 字段 | 类型 | 必选 | 说明 |
|------|------|------|------|
| eventType | enum | 是 | 审计事件类型：LOGIN_SUCCESS / LOGIN_FAILED / LOGOUT / TOKEN_REFRESH_SUCCESS / TOKEN_REFRESH_REJECTED / PASSWORD_CHANGED / ACCOUNT_LOCKED |
| userId | Long | 否 | 操作人用户 ID，未知时为 null（如登录失败时用户名不存在） |
| username | String | 否 | 操作人用户名，未知时为 null |
| clientIp | String | 是 | 客户端 IP（通过 RequestHelper.getClientIp() 获取） |
| success | boolean | 是 | 操作结果，true=成功 / false=失败 |
| failureReason | String | 否 | 失败原因，仅 success=false 时填充 |
| refreshTokenMasked | String | 否 | refreshToken 掩码（仅前 8 位 + "***"），登出/刷新时携带 |
| newJti | String | 否 | 新签发 Token 的 jti，Token 刷新成功时填充 |
| timestamp | long | 是 | 事件时间戳（epoch millis），由事件构造时自动生成 |

#### Phase 1 实现：`LoggingSecurityAuditLogger`

Phase 1 采用日志文件实现，将审计事件以结构化格式写入专用审计日志文件：

- **日志文件路径**：`${LOG_DIR}/audit/security-audit.log`（可通过配置属性 `audit.security.file-path` 覆盖）
- **日志格式**：`timestamp=<ISO-8601> eventType=<type> userId=<id> username=<name> clientIp=<ip> success=<bool> [failureReason=<reason>] [refreshTokenMasked=<mask>] [newJti=<jti>]`
- **日志滚动**：依赖 Logback 的 `RollingFileAppender`，按天滚动，保留 90 天
- **线程安全**：单例 Bean，无状态；Logback 内部已保证 append 操作的线程安全
- **异常处理**：捕获 `IOException` 等异常，记录到业务日志（`log.warn("Audit log write failed: {}", e.getMessage())`），不向调用方抛出

#### 调用入口约定

- `AuthServiceImpl` 在以下方法中调用 `SecurityAuditLogger.logAudit(...)`：
...
- `AuthServiceImpl` 注入 `SecurityAuditLogger`（通过 `AuthModuleConfig` 注册为 `@Bean`），不直接在 Controller 层调用

#### Phase 2 迁移路径

Phase 2 可平滑迁移至数据库审计表实现（如 `sys_security_audit_log` 表），仅需新增 `DatabaseSecurityAuditLogger` 实现 `SecurityAuditLogger` 接口，在 `AuthModuleConfig` 中通过 `@ConditionalOnProperty(name = "audit.security.storage", havingValue = "db")` 切换 Bean。业务代码（`AuthServiceImpl`）不需任何变更。

#### 与业务日志的关系

- **业务日志**（`log.info` / `log.warn`）：面向开发/运维，记录业务流程和异常信息
- **审计日志**（`SecurityAuditLogger`）：面向安全审计/合规，记录安全敏感操作的完整追溯信息
- **不允许混用**：安全敏感操作必须通过 `SecurityAuditLogger` 记录，不能仅用 `log.info` 代替；`log.info` 不满足审计要求
...
| component 字段映射 | 实体未声明 `component` 字段，但 `sys_function` 表存在 `component` 列 | 新增 `@Column(name = "component") private String component;` 字段，添加 getter/setter；schema.sql 已存在 `component VARCHAR(...)` 列（存储前端组件路径如 `Layout`、`system/user/index`），无需 DDL 变更 |
...
**User → UserInfoResponse 转换的唯一来源**：`UserConverter` 是 `User` 实体与 `UserInfoResponse` DTO 之间转换逻辑的唯一来源。`UserFacadeImpl.toUserInfoResponse(User user)` **必须委托** `userConverter.toUserInfoResponse(user)` 完成转换，禁止在 `UserFacadeImpl` 中独立实现 `User → UserInfoResponse` 转换逻辑。`UserConverter` 中负责以下三个关键转换步骤：
- **角色解析**：按 `Role::getEnabled` 过滤已禁用角色；按 `Role::getSort` 升序取首个，使用 `Comparator.nullsLast(Comparator.naturalOrder())` 防止 sort 为 null 时 NPE
- **权限解析**：按 `PermissionFunction::getEnabled` 过滤已禁用权限
- **字段映射**：`User.nickname` → `realName`、主角色 code → `role`、岗位 code → `position`

`UserFacadeImpl` 仅承担数据访问职责（调用 `UserRepository.findById` / `findByUsername`），不重复实现转换逻辑。避免两处独立维护 `User → UserInfoResponse` 转换导致的不一致（如某处忘记过滤已禁用角色或权限）
...
| ACCOUNT_LOCKED | ACCOUNT_LOCKED | 模板消息 `"账户已锁定，请{锁定时间}后重试"`，运行时由 `GlobalExceptionHandler` 完成插值：IP 维度锁定 → "账户已锁定，请 30 分钟后重试"；用户名维度锁定 → "账户已锁定，请 15 分钟后重试" | 登录失败超限锁定，args 传递锁定时长文本。详细插值管线见 10.3 节 |
...
部分 ErrorCode 的消息需根据运行时上下文动态生成（如 `ACCOUNT_LOCKED` 的锁定时间随锁定维度不同而不同）。为统一管理动态消息，需定义完整的消息模板插值管线，覆盖从 `BusinessException` 构造到 `Result` 输出的全链路。
...
BusinessException 构造
    ↓ (args 传递动态参数)
Service 层抛出 (如 AuthServiceImpl.login())
    ↓
GlobalExceptionHandler.handleBusinessException() 统一出口
    ↓ (模板插值)
...
1. **ErrorCode 消息字段为模板字符串**：消息文本中的占位符使用 `{参数名}` 格式。例如 `ACCOUNT_LOCKED` 的消息定义为 `"账户已锁定，请{锁定时间}后重试"`，占位符为 `{锁定时间}`。

2. **args 在 Service 层传入**：`AuthServiceImpl` 等业务层在检测到锁定后构造 `BusinessException` 时，将动态参数作为 args 传入：
   ```java
   // IP 维度锁定
...
3. **插值时机在 GlobalExceptionHandler**：所有 `BusinessException` 的统一出口是 `GlobalExceptionHandler.handleBusinessException()`，插值操作在此处完成。这样 Service 层不需要关心最终消息文本，只负责抛出带 args 的异常；模板格式的演进（如未来切换为 i18n 资源文件）只需在 Handler 层修改。

4. **插值方法**：
   - **简单占位符**（如 `{锁定时间}`）：使用 `String.replace("{锁定时间}", String.valueOf(args[0]))` 完成替换
   - **复杂占位符**（多参数或条件分支）：使用 `MessageFormat.format(template, args)` 处理，`MessageFormat` 支持 `{0}` `{1}` 等位置参数
   - **混合支持**：优先使用 `MessageFormat`（支持位置参数和格式选项），若模板仅含 `{参数名}` 形式则降级为 `String.replace`

5. **args 为 null/空时的处理**：
   - args 为 null 或长度为 0 → 不执行插值，直接返回模板原文
   - 占位符在 args 中找不到对应值 → 保留占位符原文（不抛异常），便于调试时识别模板缺失
   - 占位符数量 > args 数量 → 保留未填充的占位符；占位符数量 < args 数量 → 忽略多余 args
...
6. **Result 响应格式**：
   ...
   ```
   `message` 字段为插值后的完整文本，前端直接展示给用户，无需自行处理占位符。
...
| ErrorCode | 模板消息 | args[0] 取值 | 触发场景 |
|-----------|---------|------------|---------|
| ACCOUNT_LOCKED | "账户已锁定，请{锁定时间}后重试" | "30分钟" / "15分钟" | IP 维度锁定（30 分钟）/ 用户名维度锁定（15 分钟） |

#### 扩展性约束

- 新增需要动态消息的 ErrorCode 时，必须：(a) 在 OOD 10.2 节中明确模板格式和 args 取值；(b) 在 `GlobalErrorCode` 枚举的消息字段中使用占位符；(c) 业务层抛出异常时传入对应 args；(d) `GlobalExceptionHandler` 通用插值逻辑自动支持，无需修改
- 严禁绕过 `GlobalExceptionHandler` 在 Service 层预先插值后硬编码消息（破坏插值管线的统一性）
...
| 安全审计日志实现 | log.info vs SecurityAuditLogger 接口 | SecurityAuditLogger 接口 + Phase 1 日志文件实现 | 审计日志需满足不可篡改、结构化、可追溯，`log.info` 不满足审计要求；通过接口抽象支持 Phase 2 平滑迁移至数据库审计表 |
| 错误消息动态生成 | Service 层硬编码 vs 模板插值 | `ErrorCode` 消息字段为模板，args 传递动态参数，`GlobalExceptionHandler` 统一出口完成插值 | 统一插值管线避免 Service 层硬编码导致的格式不一致；支持未来扩展其他动态消息 ErrorCode；模板格式演进（如 i18n）只需修改 Handler 层 |
| User → UserInfoResponse 转换职责 | UserFacadeImpl 自实现 vs 委托 UserConverter | `UserFacadeImpl.toUserInfoResponse()` 必须委托 `UserConverter` | 转换逻辑唯一来源，避免两处独立维护导致的不一致（如某处忘记过滤已禁用角色/权限）；`UserConverter` 作为 `@Component` 单一职责类，更易测试和维护 |
...
| 3.12 | `auth/audit/SecurityAuditLogger.java` + `LoggingSecurityAuditLogger.java` + `SecurityAuditEvent.java` | 新建 | OOD 4.8 + v22 T8 |
