# Phase 0 最小化骨架 OOD 设计方案 — 诊断报告（v8）
...
- **偏离说明**：需求文档定义了完整的"角色—岗位—功能"三级权限可访问矩阵（含 7 类角色 × 28 项功能的 ✓/✗/○¹/○² 映射），并明确定义了 `○¹`（创建者/责任人）与 `○²`（接诊/经手人）两种受限访问语义。OOD 的权限模型设计（§3.3）虽然正确地采用了三级模型骨架，但存在以下缺口：
  ...
  - 需求 §2.6 中"线下接诊医生"角色（2.3.5）在 OOD 的 §3.3 权限模型框架中已有建模能力支撑（User/Role/Post/Function 四级模型可承载该子岗位的角色定义），具体角色数据初始化属于 Phase 1 数据种子任务，非 Phase 0 OOD 设计缺失。建议在 §3.3 末尾补充对 Phase 1 数据种子中角色/岗位枚举的指引说明，标注 Phase 1 开发者需将"线下接诊医生"纳入 `Post` 数据种子枚举。
...
- **风险提示（P1）**：该超前实现中包含的 `PasswordEncoder` Bean 在 Phase 0 的 `permitAll` 模式下不会被实际调用。若 `PasswordEncoder` 配置存在错误（如编码器算法选择不当、BCrypt 强度参数异常），该错误将在 Phase 0 被隐藏，直到 Phase 1 启用认证时才暴露。该风险隐蔽性高、影响范围大（认证环节的核心组件），优先级判定为 P1——虽然不阻塞 Phase 0 验收标准，但配置错误在 Phase 1 启用认证时可能暴露为难以追溯的认证失败问题，且 Phase 0 阶段排查该配置无额外成本。建议在 Phase 0 中添加独立的 `PasswordEncoder` 单元测试（无需集成 SecurityFilterChain），以验证 Bean 创建和基本编码/匹配功能正常，避免配置错误被延迟暴露至 Phase 1。
...
- **修复者须知**：OOD 应补充一份"实体交付阶段映射表"——列出全部 18+ 核心实体及其归属的阶段，避免后续开发者遗漏 Phase 1 需要准备的关键实体骨架。**放置说明**：该映射表不属于 Phase 0 OOD 修改范围，建议另建独立文档（如 `Docs/entity_stage_mapping.md`），由项目管理者确认生命周期和更新责任人。该独立文档作为 Phase 1 搭建期的参考材料，避免对 OOD 的持续性维护负担。
...
- **修复者须知**：OOD 应在 §8.2 各 AI 能力 DTO 定义处补充对应的超时阈值（引用需求 §3.4.x），同时在 §6 并发设计中标注"各 AI 能力超时阈值见 §8.2 对应 DTO 定义"。§8.2 提供具体数值索引，§6 提供超时控制机制的架构级上下文，两处修改均需执行，不可偏废其一。**实现区分**：超时阈值的嵌入方式有两种可选形态——（A）DTO 字段注释形态（阈值标注在每个 DTO 的 Java 字段注释中，与 DTO 定义同位置）和（B）独立索引表形态（在 §8.2 末尾或附录中增加"AI 能力超时阈值索引表"）。权衡建议：推荐路径（B）独立索引表，理由：（1）超时阈值是运行时配置属性，不属于接口契约的结构部分，与 DTO 耦合在信息架构层面不纯粹；（2）路径（A 注释形态）的操作复杂度和后期维护成本高于路径（B）——每处 DTO 字段注释需单独编辑，而独立索引表只需维护一处。
