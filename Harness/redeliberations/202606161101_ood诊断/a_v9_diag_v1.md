# Phase 0 最小化骨架 OOD 设计方案 — 诊断报告（v9）
...
- **修复者须知**：OOD 应在 §8.2 各 AI 能力 DTO 定义处补充对应的超时阈值（引用需求 §3.4.x），同时在 §6 并发设计中标注"各 AI 能力超时阈值见 §8.2 对应 DTO 定义"。§8.2 提供具体数值索引，§6 提供超时控制机制的架构级上下文，两处修改均需执行，不可偏废其一。超时阈值的嵌入方式建议采用独立索引表形态（在 §8.2 末尾或附录中增加"AI 能力超时阈值索引表"），避免与 DTO 接口契约结构耦合。
