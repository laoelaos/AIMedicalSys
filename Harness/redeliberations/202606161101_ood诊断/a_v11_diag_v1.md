# Phase 0 最小化骨架 OOD 设计方案 — 诊断报告（v11）
...
- **修复者须知**：OOD 应在 §8.2 各 AI 能力 DTO 中补充对应错误码清单。
...
- **降级依据**：权限矩阵的角色映射缺口属于数据种子和验证清单层面，不影响骨架运行和编译，不阻塞 Phase 0 三项验收标准中的任何一项。DataPermissionEvaluator 语义覆盖缺口属于扩展点预留的设计完备性检查——OOD §3.3 已预留该扩展点，Phase 1 实现时需验证其接口签名能承载 ○¹/○² 的语义差异，同属 Phase 0→Phase 1 衔接准备项，不阻塞 Phase 0 验收标准。
- **修复者须知**：建议在 OOD §3.3 中补充权限矩阵与需求 §2.6 的对齐验证结果，明确各角色访问路径的覆盖情况。
...
- **修复者须知**：OOD 应在 §8.2 的 DTO 字段描述中补充各字段的必填/长度约束（从需求 §3.4.x 直接引用），并在 §8.2 统一约束补充部分（L1272）增加字段约束索引表。
...
- **风险提示（P1）**：该超前实现中包含的 `PasswordEncoder` Bean 在 Phase 0 的 `permitAll` 模式下不会被实际调用。若 `PasswordEncoder` 配置存在错误（如编码器算法选择不当、BCrypt 强度参数异常），该错误将在 Phase 0 被隐藏，直到 Phase 1 启用认证时才暴露。该风险隐蔽性高、影响范围大（认证环节的核心组件），优先级判定为 P1——虽然不阻塞 Phase 0 验收标准，但配置错误在 Phase 1 启用认证时可能暴露为难以追溯的认证失败问题，且 Phase 0 阶段排查该配置无额外成本。建议在 Phase 0 中添加独立的 `PasswordEncoder` 单元测试（需在测试中激活 `!phase0` Profile 以触发 Bean 创建，因 `PasswordEncoder` 配置于 `@Profile("!phase0")` 下，Phase 0 默认 Profile 下 Bean 不会被创建），以验证 Bean 创建和基本编码/匹配功能正常，避免配置错误被延迟暴露至 Phase 1。测试上下文应限定为仅加载 PasswordEncoder 相关的配置切片（如使用 `@SpringBootTest(classes = {SecurityConfig.class})` 指定加载类，或使用 `@TestConfiguration` 内联定义），避免 `!phase0` Profile 连带创建其他 Phase 1 Bean 导致测试上下文加载失败。
...
- **修复者须知**：建议另建独立文档（如 `Docs/entity_stage_mapping.md`）作为"实体交付阶段映射表"——列出全部 18+ 核心实体及其归属的阶段，避免后续开发者遗漏 Phase 1 需要准备的关键实体骨架。该映射表不属于 Phase 0 OOD 修改范围，由项目管理者确认生命周期和更新责任人，作为 Phase 1 搭建期的参考材料，避免对 OOD 的持续性维护负担。
...
- **修复者须知**：建议在 §8.2 各 AI 能力 DTO 定义处补充对应的超时阈值（引用需求 §3.4.x），同时在 §6 并发设计中标注"各 AI 能力超时阈值见 §8.2 对应 DTO 定义"。§8.2 提供具体数值索引，§6 提供超时控制机制的架构级上下文，建议同步进行。超时阈值的嵌入方式建议采用独立索引表形态（在 §8.2 末尾或附录中增加"AI 能力超时阈值索引表"），避免与 DTO 接口契约结构耦合。

### 8.3 权限模型 Phase 1 数据种子与跨模块授权预留（P1）

- **来源**：需求 §2.6 功能-角色可访问矩阵中两项原归类于 §2.2 的缺口，经重新分类确认属于 Phase 1+ 衔接准备项，非 Phase 0 OOD 的实质性偏离。
...
- **修复者须知**：以上两项属于 Phase 1 数据种子和权限扩展的实现参考，不影响 Phase 0 验收标准。建议 Phase 1 开发者将本节作为参考备忘录：（1）在数据种子初始化逻辑中将"线下接诊医生"纳入 `Post` 枚举定义；（2）在实现 `DataPermissionEvaluator` 扩展点时，将药房医生查看检查/检验结果的跨模块授权路径纳入测试用例覆盖范围。
