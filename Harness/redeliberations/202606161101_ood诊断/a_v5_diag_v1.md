# Phase 0 最小化骨架 OOD 设计方案 — 诊断报告（v5）
...
> **诊断边界说明**：MySQL/PostgreSQL 驱动 scope 不一致项涉及 OOD 对 Phase 1+ 配置的引用描述（Phase 0 仅使用 H2 内存数据库）。该问题不影响 Phase 0 交付物的运行正确性，但同一文档中同类依赖（JDBC 驱动）使用不同 scope（`runtime` vs `compile`）属于依赖管理风格不一致。`compile` 在叶子模块中虽无 transitive 传播风险，但不一致的 scope 策略会给后续维护者造成困惑。建议统一为 `runtime` 以保持文档一致性和最佳实践。
...
- **修复者须知**：OOD 应补充一份"实体交付阶段映射表"——列出全部 18+ 核心实体及其归属的阶段，避免后续开发者遗漏 Phase 1 需要准备的关键实体骨架。
...
- **修复者须知**：OOD 应在 §8.2 各 AI 能力 DTO 定义处补充对应的超时阈值（引用需求 §3.4.x），同时在 §6 并发设计中标注"各 AI 能力超时阈值见 §8.2 对应 DTO 定义"。§8.2 提供具体数值索引，§6 提供超时控制机制的架构级上下文，两处修改均需执行，不可偏废其一。
