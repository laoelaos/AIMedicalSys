# Phase 0 最小化骨架 OOD 设计方案 — 诊断报告（v2）
...
### 3.1 路线图要求的协作规范在 OOD 中完全缺失（P1）

- **位置**：路线图 §Phase 0 "做完后新增什么能力"→ "协作规范"
- **偏离**：路线图将"协作规范：分支约定、Commit 格式、PR 模板、Code Review 必查项"列为 Phase 0 骨架必备交付项之一。OOD 文档未包含任何协作规范内容。
- **责任边界**：协作规范是团队层面的工程纪律文档，应由项目管理者或技术负责人创建独立的 `CONTRIBUTING.md` 或 `docs/` 下的协作规范文档，不属于 OOD 架构设计文档的补充范围。OOD 作为技术设计文档，职责边界在于模块划分、接口契约、实体设计等技术事项。因此协作规范缺失的责任方不是 OOD 作者，而是项目管理者未在 Phase 0 交付物中包含独立协作规范文档。

### 3.2 路线图"推荐补齐"项有遗漏（P2）

- **位置**：路线图 §Phase 0 "推荐补齐"
- **偏离**：路线图列出以下推荐补齐项，OOD 文档未涉及：
  - "容器化开发部署脚本"（容器编排配置，支持本地一键启动开发环境）
  - "本地代码质量检查工具集成"（代码规范检查工具配置）
- **说明**：上述两项为推荐补齐项（"可在后续阶段首期补齐"），不阻塞骨架验收。但 OOD 作为 Phase 0 设计文档应至少提及或预留扩展点（如 §9.3 一键启动后补充 Docker Compose 的占位路径，或 §10 CI 占位后补充本地 lint 工具配置的预留位置），以便修复者知晓该方向的需求。

---

## 4. 反向边界检查（新增维度）

### 4.1 DTO 超前定义带来的需求稳定性风险（P1）

- **位置**：OOD §8.2 — 全部 13 项 AI 能力的完整字段级 DTO 定义
- **检查发现**：OOD §8.2 为全部 13 项 AI 能力定义了完整的字段级 DTO（含输入/输出契约），其中约半数能力的真实实现归属 Phase 2~5（如 3.4.4 AI 智能诊断归 Phase 4、3.4.7 AI 影像分析归 Phase 4、3.4.8 AI 知识库问答归 Phase 5、3.4.12 AI 医生排班归 Phase 5、3.4.13 AI 综合讨论结论归 Phase 5）。这些 DTO 在 Phase 0 均以 `MockAiService` 的固定占位数据形式存在。
- **风险分析**：
  - **需求稳定性风险**：需求 §3.4.x 的字段级契约在 Phase 5 实际落地前可能发生变更（如 `KgQueryRequest` 增加 `department_id` 字段、`ScheduleResponse` 调整结构等）。Phase 0 冻结的 DTO 与实际落地时的需求之间存在偏差风险，届时需回溯修订 OOD 和已有 Mock 实现。
  - **前期维护成本**：当前阶段定义全部 13 项 DTO 迫使开发者在骨架阶段理解全部 AI 能力的字段细节，增加了认知负荷和文档维护量。修订 Chain（OOD → ai-api DTO → MockAiService → TypeScript 类型）任一环节变更需协调同步。
- **合理性评估**：这种超前定义在架构层面具有合理性（为模块划分提供完整的编译期契约边界，支撑前端 TypeScript 类型同步），但 OOD 未显式记录该做法的权衡理由和风险应对策略。若需求在 Phase 2~5 发生 DTO 字段变更，修复者（后端架构师）需评估变更对骨架层的影响。
- **建议**：OOD 应在 §8.2 或 §7 设计决策中补充一条 ADR，说明"为何在 Phase 0 超前定义全部 13 项 AI 能力的 DTO"，并标注潜在的需求变更风险和应对策略（如"需求变更时同步更新 ai-api DTO 和 MockAiService，前端通过 openapi-generator 同步类型定义"）。
...
### 5.1 CSRF 未禁用——Phase 1+ 的前瞻性风险（P2）
...
- **事实**：Phase 0 的 `SecurityConfigPhase0` 配置如下：
  ...
  ```
- **风险说明**：Spring Security 6+ 默认启用 CSRF 保护（`CsrfFilter`）。虽然配置了 `permitAll()`，但 CSRF 保护是针对状态变更请求（POST/PUT/DELETE）在 filter 链中先于授权检查执行的。Phase 0 当前仅有 `GET /api/ping` 端点，因此 **Phase 0 范围内不会触发该问题**——不存在任何 POST 请求被 CSRF filter 拒绝的场景。该风险仅在 Phase 1 添加第一个 POST 业务端点（如登录 `/api/login`）时才会暴露。
- **推荐处理方式**：在 Phase 0 SecurityConfig 中添加注释标记该骨架为 API 无状态认证架构、CSRF 不适用，并将 `.csrf(csrf -> csrf.disable())` 的添加操作作为 Phase 1 启动前必须完成的 Checklist 项。当前无需在 Phase 0 中添加 `csrf.disable()` 代码（因为 Phase 0 无 POST 端点，增加未验证的配置属于过度防御）。
...
| 质询意见 | 回应 |
|---------|------|
| §3.1 ASCII 依赖关系图箭头歧义被不当归为"逻辑错误"，严重程度失当 | 已从"逻辑错误"类别移除，移至 §6.C 补充说明，明确这不属于逻辑推理错误而是图示表现力限制。 |
| §2.1 CSRF 未禁用归为"事实错误"不当；Phase 0 范围内不构成事实错误 | 已从"事实错误"移除，移至 §5.1 技术风险提示，明确标注"Phase 0 范围内不触发该问题"，重新分类为前瞻性风险提示。 |
| §4 偏离需求文档覆盖深度严重不足，仅查验一个技术细节后即断言无重大偏离 | 已补充至 5 项系统验证（§2.1 错误码约定、§2.2 权限矩阵、§2.3 DTO 字段约束、§2.4 实体关系、§2.5 非功能指标），覆盖权限矩阵、DTO 字段约束、实体关系、非功能指标等维度。 |
| §5.1 协作规范缺失未区分问题归属，修复者不明责任边界 | 已在 §3.1 中明确责任边界：协作规范是独立文档（CONTRIBUTING.md）的职责，不属于 OOD 架构设计文档的补充范围，责任方为项目管理者而非 OOD 作者。 |
| 诊断报告全文未提供任何优先级排序，修复者无法确定处理顺序 | 已为全文每一项诊断结论添加 P0/P1/P2 优先级标记。 |
| §2.2 @SQLRestriction 仅对 Hibernate 查询生效，在 Phase 0 骨架中影响有限，问题归因过于放大 | 已从"事实错误"降级为 §5.2 技术风险提示，明确标注 Phase 0 范围内无实际影响，属于 Phase 2+ 开发者须知。 |
| 补充发现：未从反向评估 OOD 是否包含超出 Phase 0 范围的内容 | 新增第 4 章"反向边界检查"，评估了 §8.2 DTO 超前定义的风险（§4.1）和 SecurityConfig 超前预留的合理性（§4.2）。 |
