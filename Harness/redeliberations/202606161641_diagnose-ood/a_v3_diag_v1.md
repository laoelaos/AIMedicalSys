...
推荐采用方案 A（保留最小 SecurityConfigPhase0 permitAll 占位）以保持骨架可运行，同时将 Phase 1 专属的共享安全配置移至 Phase 1 OOD。参见上方修复方案分析。
