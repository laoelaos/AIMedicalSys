...
1. **OOD 骨架代码**中 `<dependencyManagement>` 包含 13 个条目（6 内部 + 5 starter + springdoc + h2）。
2. **实际 `<dependencyManagement>`** 仅含 11 个条目（6 内部 + 3 业务模块 + springdoc + h2），缺少 web、data-jpa、security、validation、test 五项 starter。
...
1. **优先方案**：在测试中先触发 `@PostConstruct` 启动期逻辑，使日志行为对齐设计语义后，再基于对齐后的行为编写断言（启动期 → ERROR，后续调用 → WARN）。两种实现方式及代码骨架如下：

   **方式A：反射调用 `@PostConstruct` 方法（推荐，无 Spring 上下文依赖）**

   使用 `ReflectionTestUtils`（Spring Test 自带，无需额外依赖）或标准 Java 反射触发 `FallbackAiService` 的 `@PostConstruct` 初始化方法：

   ```java
   import org.springframework.test.util.ReflectionTestUtils;

   @Test
   void shouldLogErrorOnStartupThenWarnOnRuntime() {
       // 1. 注入 Logger ListAppender
       Logger logger = (Logger) LoggerFactory.getLogger(FallbackAiService.class);
       ListAppender<ILoggingEvent> appender = new ListAppender<>();
       appender.start();
       logger.addAppender(appender);
       try {
           // 2. 触发 @PostConstruct 启动期逻辑（反射调用 init 方法）
           FallbackAiService service = new FallbackAiService();
           // 手段A-1: 使用 Spring Test ReflectionTestUtils
           ReflectionTestUtils.invokeMethod(service, "init"); // 假设 @PostConstruct 方法名为 init
           // 手段A-2: 若不需要 Spring 依赖，直接使用标准反射
           // service.getClass().getDeclaredMethod("init").invoke(service);
           ...
           // 4. 触发首次运行期调用（此时 firstEmptyDelegateCall 为 true，应输出 WARN）
           appender.list.clear();
           service.handleEmptyDelegates(); // 或 public 方法路径
           assertEquals(1, appender.list.size());
           assertEquals(Level.WARN, appender.list.get(0).getLevel());

           // 5. 触发第二次运行期调用（仍为 WARN）
           appender.list.clear();
           service.handleEmptyDelegates();
           assertEquals(1, appender.list.size());
           assertEquals(Level.WARN, appender.list.get(0).getLevel());
       } finally {
           logger.detachAndStopAppender(appender);
   ...
   **前提条件**：`@PostConstruct` 方法需对同包或反射可见（通常为 `public` 或 `package-private`）。若该方法为 `private`，反射调用前需 `setAccessible(true)`。若 `init` 方法名称未知，可通过 `FallbackAiService.class.getDeclaredMethods()` 遍历查找含 `@PostConstruct` 注解的方法。

   **方式B：Spring Test 上下文刷新**（适用于需要完整 Spring 容器场景）

   使用 `@SpringBootTest` + `@DirtiesContext(classMode = BEFORE_EACH_TEST_METHOD)` 触发完整的 Spring 容器生命周期（包括 `@PostConstruct` 回调）：

   ```java
   @SpringBootTest
   @DirtiesContext(classMode = DirtiesContext.ClassMode.BEFORE_EACH_TEST_METHOD)
   class FallbackAiServiceLoggingTest {

       @Autowired
       private FallbackAiService service;

       @Test
       void shouldLogErrorOnStartup() {
           Logger logger = (Logger) LoggerFactory.getLogger(FallbackAiService.class);
           ListAppender<ILoggingEvent> appender = new ListAppender<>();
           appender.start();
           logger.addAppender(appender);
           try {
               // 容器启动时 @PostConstruct 已自动执行
               // 验证启动期 ERROR 日志（注意：ListAppender 需在 @PostConstruct 执行前注入，
               // 因此仅靠 @PostConstruct 无法在测试方法中捕获启动期日志——实际需通过
               // ApplicationListener<ApplicationReadyEvent> 或 @BeforeAll 替代方案）
               // 因此方式A 是更实际的选择
           } finally {
               logger.detachAndStopAppender(appender);
           }
   ...
   **方式B 的局限性**：`ListAppender` 在 `@PostConstruct` 已执行完毕后才被注入测试类，无法捕获启动期日志。如需通过 Spring 上下文捕获启动期日志，应采用 `@TestExecutionListener` 或 `ApplicationRunner` 在容器启动阶段注入 Appender，复杂度显著增加。因此**推荐方式A（反射调用）** 作为首选实现。

2. **备选方案**：如果保持对当前代码行为的断言，必须在 ListAppender 代码骨架旁显式注释 "⚠️ 此断言依赖 K3 已知偏差，K3 修复后需同步更新"
...
5. **当前代码分析**：`types/index.ts` 的 `ApiResult<T>` 结构与修复后错误拦截器的返回格式兼容（`code: string` + `message?: string`），若选择方案B，需将成功拦截器从 `return response.data.data` 调整为 `return response.data`（即返回完整的 `ApiResult<T>`），使消费方统一以 `ApiResult<T>` 类型消费。此调整属于修复的连带变更，需纳入修复范围。
