...
P0(Phase 1+) / P2(Phase 0)（Phase 1+ 场景下的最高优先级 — 引入 POST/PUT 请求体后，请求体 JSON 格式错误时将错误返回 500 而非 400，前端可能被错误引导；当前 Phase 0 下：所有 Controller 均为 `@GetMapping` 且不接收 `@RequestBody`，因此 `HttpMessageNotReadableException` 不存在触发路径，无运行时影响）。
...
1. **优先方案**：在测试中先触发 `@PostConstruct` 启动期逻辑（如通过反射调用或 Spring Test 上下文刷新），使日志行为对齐设计语义后，再基于对齐后的行为编写断言（启动期 → ERROR，后续调用 → WARN）
...
P0(Phase 1+) / P2(Phase 0)（Phase 1+ 场景下的最高优先级 — 真实的 API 调用链路引入后，前端无法通过统一 code 判断网络错误，未捕获的 Promise 异常可能产生运行期报错；当前 Phase 0 下：前端仅含占位页面和健康检查，无实际 API 调用链路会触发网络错误，故无运行时影响）。
...
| 8 | P0(Phase 1+)/P2(Phase 0) | GlobalExceptionHandler 缺少序列化异常处理器 | 代码缺陷（类型①） | 否 | todo 第8项 | `GlobalExceptionHandler.java` 功能遗漏 |
| 9 | P2 | FallbackAiServiceTest 未验证日志输出 | 测试覆盖不充分（类型④） | 否 | todo 第9项 | `FallbackAiServiceTest.java` 未覆盖日志（关联 known_issues.md K3） |
| 10 | P0(Phase 1+)/P2(Phase 0) | ApiClient 错误拦截器未实现 NETWORK_ERROR | 代码缺陷（类型①） | 否 | todo 第10项 | `api/index.ts` 功能遗漏 |
...
第3步（可与第2步并行，且完全独立于第1步）：
  ├── 修复代码缺陷：
  │   ├── 问题8（P0(Phase 1+)，完全独立，可最先执行）
  │   └── 问题10（P0(Phase 1+)，完全独立，可最先执行）
  ├── 修复 POM 配置：
  │   ├── 问题7（P1，需验证 CI 门禁，见 CI 门禁影响分析）
  │   ├── 问题5（P2，纯补充/清理，无需额外验证）
  │   └── 问题6（P2，纯补充/清理，无需额外验证）
  └── 修复测试 & 文档：
      ├── 问题9（P2，完全独立，不依赖任何前置步骤，可随时启动）
      └── 问题1（P2，完全独立，不依赖任何前置步骤，可随时启动）
```

**并行机会说明**：
- **问题8、问题10、问题9、问题1 与第1步无任何依赖关系**，可独立在任何时间启动（包括在第1步决策完成之前），不因决策延迟而阻塞。建议优先分配这些独立项以最大化并行度。
