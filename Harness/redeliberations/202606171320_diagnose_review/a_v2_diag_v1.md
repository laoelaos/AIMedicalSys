...
P1（中优先级 — 涉及全仓库最大结构性偏离，同时关联问题3，需先决策再行动）。
...
P0（最高优先级 — 直接影响运行时行为：请求体 JSON 格式错误时错误返回 500 而非 400，前端可能被错误引导）。
...
P0（最高优先级 — 直接影响运行时行为：前端无法通过统一 code 判断网络错误，未捕获的 Promise 异常可能产生运行期报错）。
...
| 序号 | 优先级 | 问题摘要 | 分类 | 是否为误报 | 根因所在 |
|------|--------|---------|------|-----------|---------|
| 1 | P2 | BaseEnum 未在 §1.3 定义 | OOD 文档不完善 | 否 | `Docs/04_ood_phase0.md` §1.3 |
| 2 | P1 | POM modules 路径与设计 §2.1 不一致 | 代码与设计双向偏离 | 否 | 代码目录结构 vs 设计目录约定 |
| 3 | P1 | 聚合 POM 缺失 | 代码偏离设计（问题2 的连带后果） | 否 | 代码目录结构 vs 设计目录约定 |
| 4 | P2 | Spring Boot 版本 3.2.5 vs 3.3.0 | 代码与设计版本偏离 | 否 | 版本决策未同步（含 §1.4 运行时环境表） |
| 5 | P2 | dependencyManagement 缺少 starter 声明 | 代码偏离设计 | 否 | `pom.xml` 未按骨架实现 |
| 6 | P2 | Common POM 多引入 validation (optional) | 代码偏离设计 | 否 | `common/pom.xml` 额外依赖 |
| 7 | P1 | maven-dependency-plugin 额外忽略 business | 代码与设计偏离 | 否 | `pom.xml` 豁免配置过宽 |
| 8 | P0 | GlobalExceptionHandler 缺少序列化异常处理器 | 代码缺陷 | 否 | `GlobalExceptionHandler.java` 功能遗漏 |
| 9 | P2 | FallbackAiServiceTest 未验证日志输出 | 测试覆盖不充分 | 否 | `FallbackAiServiceTest.java` 未覆盖日志（关联 known_issues.md K3） |
| 10 | P0 | ApiClient 错误拦截器未实现 NETWORK_ERROR | 代码缺陷 | 否 | `api/index.ts` 功能遗漏 |

### 修复者指引

- **修复代码**：问题 8、10 需优先修改代码（P0）；问题 7 建议按设计缩窄豁免范围（P1）；问题 5、6 按设计对齐代码或由设计侧确认后保留（需同步更新 OOD）；问题 2、3、4 需在代码重构与 OOD 更新间二择其一（P1/P2）。
- **修复 OOD 文档**：问题 1 需在 §1.3 补充 BaseEnum 定义（P2）；若选择降版本方案，问题 4 需同步更新 §1.4 的版本引用。
- **修复测试**：问题 9 需在测试中追加日志验证（P2），可参考 known_issues.md K3 记录的已知偏差。
- **先决策**：问题 2/3（目录结构）涉及重大重构，需先做技术决策再开展修复。
