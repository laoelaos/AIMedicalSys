# Phase 1 包 A/B/D 统一修复与包 B OOD 设计方案（v13）
...
1. 前端提交 `{ refreshToken }` 到 `POST /api/auth/refresh`，请求**不应携带** `Authorization` header（refresh 端点为 permitAll，不依赖前置 JWT 认证；携带旧 Access Token 可能触发 401 干扰刷新流程）
