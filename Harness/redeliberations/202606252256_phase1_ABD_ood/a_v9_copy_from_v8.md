...
10. 清除当前请求 SecurityContext（作为安全最佳实践），客户端可继续使用旧 Access Token 至其自然过期（最长 15 分钟）
11. 返回成功响应
...
**全局 IP 限流（GlobalRateLimitFilter）**：对所有 API 路径（排除白名单：`/api/auth/login`、`/api/auth/refresh`、`/actuator/health`、`/actuator/info`）实施 IP 级限流。实现为 OncePerRequestFilter，在 JwtAuthenticationFilter 之前执行。白名单路径直接 `chain.doFilter` 放行；非白名单路径检查滑动窗口计数器（100 次/60 秒/IP），超限返回 429 + ErrorCode.RATE_LIMITED。GlobalRateLimitFilter**不委托** `InMemoryRateLimitGuard`，而是独立维护自己的滑动窗口计数器（同样基于 `ConcurrentHashMap<String, SlidingWindowCounter>`），与 `InMemoryRateLimitGuard`（针对登录端点 `/api/auth/login` 的专用限流，阈值 5 次/10 秒）职责分工明确：全局限流覆盖所有 API 路径（白名单除外），登录专用限流仅覆盖登录端点。两者共用滑动窗口算法实现，但计数器实例独立，互不干扰。提取公共滑动窗口工具类 `SlidingWindowCounter` 供两处复用。
...
**"连续"语义定义**：IP 维度的"连续失败"定义为同一 IP 发起的登录请求连续失败，中间不得插入任何非登录请求或登录成功。以下情况均视为中断连续序列：(a) 该 IP 发起的非登录请求（如刷新 token、获取用户信息等），无论响应状态如何；(b) 该 IP 发起的任何成功登录。中断后失败计数重置。用户名维度的"连续失败"语义相同：同一用户名连续失败的登录请求，中间插入任何非登录请求或登录成功均中断连续序列。
