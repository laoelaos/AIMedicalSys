# Phase 1 包 A/B/D 统一修复与包 B OOD 设计方案（v9）
...
**更新语义**：采用局部更新语义，请求体中省略的字段保持不变，不覆盖为空值。**一致性校验**：`PUT /api/menu/{id}` 的路径参数 `{id}` 与请求体中的 `id` 字段必须相同。Controller 层在校验逻辑中比对二者，不一致时返回 400（PARAM_INVALID）。
