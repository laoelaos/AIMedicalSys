...
5. `Optional` 为空 → 递增 LoginAttemptTracker **IP 维度**的失败计数（因无有效用户名作为 key） → 抛出 BusinessException（ErrorCode.LOGIN_FAILED，消息"用户名或密码错误"）
...
8. 返回 TokenRefreshResponse（仅含 accessToken、refreshToken、tokenType、expiresIn，不含用户信息）
9. 前端收到刷新响应后，调用 `GET /api/auth/me` 获取最新用户信息（角色、权限等），更新本地用户状态
