# Phase 1 包 A/B/D 统一修复与包 B OOD 设计方案（v4）
...
5. `Optional` 为空 → 递增 LoginAttemptTracker 失败计数 → 抛出 BusinessException（ErrorCode.LOGIN_FAILED，消息"用户名或密码错误"）
6. 用户 `enabled == false` 或 `deleted == true` → 递增 LoginAttemptTracker 失败计数 → 抛出 BusinessException（ErrorCode.LOGIN_FAILED，消息"用户名或密码错误"）
7. `passwordEncoder.matches(password, user.password)` 失败 → 递增 LoginAttemptTracker 失败计数 → 抛出 BusinessException（ErrorCode.LOGIN_FAILED，消息"用户名或密码错误"）
...
**设计要点**：步骤 5/6/7 使用统一的 ErrorCode.LOGIN_FAILED 和消息"用户名或密码错误"，不泄露用户具体状态（是否存在、是否禁用）。此策略消除了用户名可枚举的安全风险。
...
5. 签发新的 Access Token（15 分钟）和 Refresh Token（7 天，新 jti），旧 Refresh Token 的黑名单操作说明见 4.2 节
6. 从 DB 重新加载用户最新状态（enabled、roles、posts）
7. 若用户已被禁用，返回 LOGIN_FAILED 错误并强制前端清除本地 token
8. 返回 TokenRefreshResponse

**刷新时机**：前端 axios 响应拦截器监测到 401 时，自动触发静默刷新。刷新成功后重放原始请求，失败则清除认证数据并跳转登录页。

**安全说明**：Phase 1 中，旧 Refresh Token 在轮换后未加入黑名单，因此旧 token 技术上仍可被重复使用。服务端不依赖于旧 Refresh Token 的一次性语义来保证安全。详细的风险补偿措施见 4.2 节。
...
PasswordChangeCheckFilter.doFilterInternal(request, response, chain):
  ...
  3. 从 DB 检查用户 passwordChangeRequired 状态：
     a. false → chain.doFilter 放行
     b. true → 检查请求路径是否在白名单中：
        - /api/auth/password（PUT）→ chain.doFilter 放行
        - /api/auth/logout（POST）→ chain.doFilter 放行
        - 其他 → 清除 SecurityContext，返回 403 + "请先修改密码"
```

**设计要点**：此 Filter 在 JwtAuthenticationFilter 之后执行，确保只有已通过 JWT 鉴权的请求才会进入密码变更检查。白名单之外的请求返回 403（FORBIDDEN）而非 401（UNAUTHORIZED），区分"未认证"和"已认证但受业务规则限制"两种场景。
...
实现方式：`InMemoryRateLimitGuard` 使用 `ConcurrentHashMap<String, SlidingWindowCounter>`，键为 IP 地址，值为滑动窗口计数器。滑动窗口以 `ReentrantLock` 保护窗口内的排序集合，确保每个 IP 的窗口对象独立加锁。限流检查仅发生在登录请求，并发量低，锁竞争可忽略。生产环境 Phase 2 迁移到 Redis + Lua 脚本。
...
- **用户可主动撤销**：用户修改密码后，所有已签发的 Refresh Token 在下次使用时因密码变更而被拒绝（需设计密码变更后标记机制）
...
- `role` 对应前端角色标识（取用户主角色的 code）。主角色判定策略：按角色优先级（Role 实体的 `level` 或 `sort` 字段）排序取最高者；无角色时返回空字符串。不再引用不存在的 `primaryRole` 字段
...
| UserType → role 字段映射 | 取用户主角色 code 填充 role 字段 | 按角色优先级（Role 实体的 `level` 或 `sort` 字段）排序取最高者；无角色时返回空字符串。不依赖 `primaryRole` 字段 |
...
| 密码变更检查阻断 | PasswordChangeCheckFilter 返回 | FORBIDDEN | 403 |
...
| FORBIDDEN | FORBIDDEN | "无权限访问" | 角色权限不足 / passwordChangeRequired 阻断 |
| LOGIN_FAILED | LOGIN_FAILED | "用户名或密码错误" | 登录流程中用户名不存在/密码错误/账户禁用或删除 |
| ACCOUNT_DISABLED | ACCOUNT_DISABLED | "账户已被管理员停用" | 已认证请求中 JwtAuthenticationFilter 发现 enabled=false |
...
- `ACCOUNT_DISABLED` 的使用范围限定为"已认证请求中账户被禁用"场景（JwtAuthenticationFilter 中查库发现 enabled=false），此时可给出更具体的消息因攻击者已在认证之后
...
| 主角色判定 | 依赖 primaryRole 字段 vs 按角色优先级 | 按角色优先级排序 | 避免新增冗余字段；Role 实体的 level/sort 字段已足够表达优先级语义 |
