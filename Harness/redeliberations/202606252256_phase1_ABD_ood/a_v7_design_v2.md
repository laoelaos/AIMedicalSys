...
6. 若用户已被禁用或被删除，返回 TOKEN_REFRESH_FAILED 错误并强制前端清除本地 token
...
10. 清除 SecurityContext（强制用户重新登录获取新令牌）
11. 返回成功响应

**设计说明**：
- Access Token 不检查 tokenVersion 是已知设计决策，非疏忽：Access Token 有效期仅 15 分钟，短窗口内即使旧 token 未被撤销，攻击面也有上限。查库验证 enabled 状态已提供基本保护。完整的 tokenVersion 检查（含 Access Token 层面）将在 Phase 2 配合 Redis 黑名单方案一并引入。步骤 10 清除 SecurityContext 后，前端需重新发起登录或刷新流程获取新令牌
...
| H6 | 循环依赖脆弱 | 顶层 import router → import store | 使用延迟加载函数替代顶层 import：通过 `() => import('@/router')` 或 `useRouter()` 获取路由实例。推荐使用 `inject('router')` 依赖注入模式：在 app 实例化时将 router 通过 `app.provide('router', router)` 注入，store 中通过 `inject('router')` 获取，避免顶层 import 导致的循环依赖。不推荐使用 `getCurrentInstance()` 方式，因其在 `<script setup>` 上下文中存在局限性 | 延迟加载函数在 setup 外调用时需注意上下文限制 | doctor/admin stores/menu.ts |
...
| 密码策略校验失败 | PasswordPolicy 拒绝 | PASSWORD_WEAK / PASSWORD_TOO_SHORT / PASSWORD_TOO_LONG / PASSWORD_CONTAINS_USERNAME / PASSWORD_COMMON | 200 |
...
| PASSWORD_COMMON | PASSWORD_COMMON | "密码过于常见" | 密码在Top 10000常见密码列表中 |
