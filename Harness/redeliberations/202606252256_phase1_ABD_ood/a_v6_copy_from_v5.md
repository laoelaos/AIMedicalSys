# Phase 1 包 A/B/D 统一修复与包 B OOD 设计方案（v6）
...
6. 用户 `enabled == false` 或 `deleted == true` → 递增 LoginAttemptTracker **用户名和 IP 双维度**的失败计数 → 抛出 BusinessException（ErrorCode.LOGIN_FAILED，消息"用户名或密码错误"）
...
**设计要点**：步骤 5/6/7 使用统一的 ErrorCode.LOGIN_FAILED 和消息"用户名或密码错误"，不泄露用户具体状态（是否存在、是否禁用）。此策略消除了用户名可枚举的安全风险。步骤 5 在用户名不存在时执行 dummy BCrypt 比对，消除与步骤 7（真实 BCrypt 比对）之间的响应时间差异，防止攻击者通过时序侧信道枚举有效用户名。
...
6. 若用户已被禁用或被删除，返回 LOGIN_FAILED 错误并强制前端清除本地 token
7. 验证 Refresh Token claims 中的 tokenVersion 与 DB 中当前 tokenVersion 一致，若不一致说明密码已变更，拒绝刷新并强制前端清除本地 token
8. 签发新的 Access Token（15 分钟）和 Refresh Token（7 天，新 jti，携带当前 tokenVersion），旧 Refresh Token 的黑名单操作说明见 4.2 节
9. 返回 TokenRefreshResponse（仅含 accessToken、refreshToken、tokenType、expiresIn，不含用户信息）
10. 前端收到刷新响应后，调用 `GET /api/auth/me` 获取最新用户信息（角色、权限等），更新本地用户状态
...
1. 前端提交当前 Access Token 到 `POST /api/auth/logout`，请求体可选携带 `refreshToken` 字段（`RefreshTokenRequest` 格式），供服务端记录登出关联的 Refresh Token
...
PasswordChangeCheckFilter.doFilterInternal(request, response, chain):
  ...
  3. 从请求 attribute 读取 passwordChangeRequired 状态（由 JwtAuthenticationFilter 步骤 5 写入）：
     ...
     b. true → 检查请求路径是否在白名单中：
        ...
        - 其他 → 清除 SecurityContext，返回 403 + ErrorCode.PASSWORD_CHANGE_REQUIRED
```
...
1. **首次登录强制修改**：种子数据中的初始密码标记 `passwordChangeRequired = true`，用户首次登录成功后，LoginResponse 中携带 `passwordChangeRequired: true` 标记，前端检测到此标记后强制跳转到修改密码页面。提交 `PUT /api/auth/password` 成功后清除标记。密码修改成功后前端恢复流程：清除 `passwordChangeRequired` 标记 → 调用 `GET /api/auth/me` 刷新用户信息 → 调用 `GET /api/menu/tree` 获取最新菜单 → 跳转到系统首页
...
**全局 IP 限流（GlobalRateLimitFilter）**：对所有 API 路径（排除白名单：`/api/auth/login`、`/api/auth/refresh`、`/actuator/health`、`/actuator/info`）实施 IP 级限流。实现为 OncePerRequestFilter，在 JwtAuthenticationFilter 之前执行。白名单路径直接 `chain.doFilter` 放行；非白名单路径检查滑动窗口计数器（100 次/60 秒/IP），超限返回 429 + ErrorCode.RATE_LIMITED。与 `RateLimitGuard`（针对登录端点基于 `InMemoryRateLimitGuard` 的专用限流）共用同一声明式接口，但全局限流为独立 Filter 以降低登录限流的性能开销。
...
- **异常刷新检测**：服务端记录每次 Refresh Token 的使用，若检测到同一用户短时间内出现多次刷新（暗示旧 token 被重复使用），记录安全日志并触发告警，管理员可据此手动处理
...
| User | tokenVersion | 新增 `private Integer tokenVersion = 0;` | `= 0` | schema.sql `NOT NULL DEFAULT 0` |
...
| `/api/auth/logout` | POST | JWT（authenticated） | 无 | 黑名单记录（仅 Access Token jti）；可选请求体携带 refreshToken |
...
| tokenVersion 新增字段 | 无此字段 | 新增 `private Integer tokenVersion = 0;` 字段 + schema.sql `sys_user.token_version INT NOT NULL DEFAULT 0`；用于密码变更后撤销旧 Refresh Token |
...
public record MenuUpdateRequest(
    @NotNull Long id,           // 路径 {id} 与请求体中 id 必须一致，否则返回 400
    String name,
    String permission,
    Long parentId,
    String path,
...
**一致性校验**：`PUT /api/menu/{id}` 的路径参数 `{id}` 与请求体中的 `id` 字段必须相同。Controller 层在校验逻辑中比对二者，不一致时返回 400（PARAM_INVALID）。
...
| POST | `/api/auth/logout` | RefreshTokenRequest（可选） | 200 | 登出，将 Access Token jti 加入黑名单；可选携带 refreshToken |
...
- 密码修改成功后恢复流程：`PUT /api/auth/password` 成功 → 清除 `passwordChangeRequired` 标记 → 调用 `GET /api/auth/me` 刷新用户信息 → 调用 `GET /api/menu/tree` 获取最新菜单 → 跳转到系统首页
...
| H5 | 菜单仅两级 | children 不递归 | convertMenusToRoutes 改为递归展平版本。路由注册策略：递归遍历任意深度菜单树，每个菜单项生成独立展平路由（不嵌套 children），所有展平路由作为一级路由注册到 `router.addRoute()`（不带父级 Layout 嵌套），path 为从根到当前菜单的完整路径（如 `/system/user`）。与 `/` 跳过滤条件的兼容关系：根菜单（parentId === null）的 path 展平后以 `/` 开头，不影响首页跳转；path === '/' 的根路由仍单独注册并跳转到 Layout，展平路由不覆盖根路由。name 唯一性保证策略：使用 `permissionCode` 按驼峰映射（如 `system:user` 映射为 `SystemUser`），运行时检测冲突时在后追加数字序号（如 `SystemUser_1`） | 展平策略不产生路由层级嵌套；name 冲突需运行时检测补偿 | createMenuStore |
| H6 | 循环依赖脆弱 | 顶层 import router → import store | 使用延迟加载函数替代顶层 import：通过 `() => import('@/router')` 或配合 `getCurrentInstance()` 获取 router 实例。具体示例：在 store 中定义 `lazyRouter` 函数 `const getRouter = () => { const { appContext } = getCurrentInstance()!; return appContext.config.globalProperties.$router; }`；或在 Pinia store 初始化时通过 `inject('router')` 依赖注入 | 延迟加载函数在 setup 外调用时需注意 getCurrentInstance() 的上下文限制 | doctor/admin stores/menu.ts |
...
| 密码变更检查阻断 | PasswordChangeCheckFilter 返回 | PASSWORD_CHANGE_REQUIRED | 403 |
...
- `PASSWORD_CHANGE_REQUIRED` 专门用于 PasswordChangeCheckFilter 对白名单之外 API 的 403 阻断，与 `FORBIDDEN`（角色权限不足）语义区分
