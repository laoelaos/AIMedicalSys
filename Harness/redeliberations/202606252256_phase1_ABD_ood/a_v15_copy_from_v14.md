# Phase 1 包 A/B/D 统一修复与包 B OOD 设计方案（v14）
...
| 审查意见 | 修改措施 |
|---------|---------|
| 问题1（重要）：Role.sort 字段缺少 Java 默认值与 NOT NULL 约束，JPA 持久化存在不一致风险 | 5.1 节 Role.java 变更表 sort 行：字段定义由 `private Integer sort;` 改为 `@Column(nullable=false) private Integer sort = 0;`，DDL 由 `INT DEFAULT 0` 改为 `INT NOT NULL DEFAULT 0`。4.3 节修复方案列表同步更新 Java 定义和 DDL 列定义。4.3 节 NOT NULL 约束状态确认表新增 Role.sort 行 |
| 问题2（重要）：SlidingWindowCounter 声明为"包级私有"但被跨包（rateLimit/ → security/）复用，存在可见性矛盾 | 4.1 节 SlidingWindowCounter 契约描述由"包级私有工具类"改为"public 工具类，放置于 `auth/rateLimit/` 包下"，消除跨包可见性矛盾。2.1 节目录结构新增 SlidingWindowCounter.java 条目 |
| 问题3（重要）：密码变更流程中清除 SecurityContext 操作无实际安全效果，说明不准确 | 3.1.6 节步骤 10 描述修正为"清除当前请求上下文，不影响后续请求的安全上下文"。设计说明第一段同步修正为"步骤 10 清除 SecurityContext 仅为当前请求的上下文清理操作，无实际安全效果；密码变更的安全保护由 tokenVersion 递增和 Access Token 短有效期提供" |
| 问题4（重要）：PasswordPolicy 接口缺少方法签名定义，无法直接指导编码 | 1.3 节 PasswordPolicy 行职责补充 `ErrorCode validate(String password, String username)` 方法签名。4.3 节新增"PasswordPolicy 接口方法签名"段落，定义参数、返回值和语义 |
| 问题5（中等）：菜单删除操作未定义子菜单处理策略（级联/拦截/置空） | 6.1 节 DELETE /api/menu/{id} 行补充"有子菜单时阻止删除，返回 400 + CHILDREN_EXIST"。10.1 节错误分类表和 10.2 节 ErrorCode 表新增 CHILDREN_EXIST 条目 |
| 问题6（中等）：expiresIn 字段语义自相矛盾——"剩余有效秒数"（动态剩余）与"从签发时计算"（固定 TTL）冲突 | 5.2 节 LoginResponse 和 TokenRefreshResponse 的 expiresIn 语义统一为"固定有效期（秒，从签发时刻起固定不变）"。6.2 节 JSON 示例注释同步修正 |
| 问题7（中等）：`/api/auth/refresh` 端点未文档化不携带 Authorization header 的调用约束 | 3.1.3 节步骤 1 补充 refresh 请求不应携带 Authorization header 的约束说明。7.2 节新增"refresh 端点调用约束"段落，明确在 axios 拦截器中显式移除 Authorization header 的要求 |
| 问题8（轻微）：刷新端点未加入 PasswordChangeCheckFilter 白名单，存在理论防护盲区 | 3.3 节 PasswordChangeCheckFilter 行为契约白名单新增 `/api/auth/refresh（POST）`。3.1.2 节已认证请求流程步骤 7 白名单列表同步补充 `/api/auth/refresh` |

## 25. 修订说明（v14）

| 审查意见 | 修改措施 |
|---------|---------|
| 问题1（严重）：Refresh 端点可绕过 passwordChangeRequired 强制约束——AuthServiceImpl.refresh() 中不存在 passwordChangeRequired 状态检查 | 3.1.3 节 Token 刷新流程步骤 5 后新增步骤 8（passwordChangeRequired 检查）：从 DB 检查 user.passwordChangeRequired，若为 true 抛出 PasswordChangeRequiredException 返回 403 + PASSWORD_CHANGE_REQUIRED，后续步骤顺延 |
| 问题2（重要）：3.4 节白名单与 3.3 节/3.1.2 节白名单不一致（内部矛盾）——3.4 节仅列出两个端点 | 3.4 节 passwordChangeRequired 访问控制白名单补充 `POST /api/auth/refresh` 条目及说明"刷新逻辑内部仍会检查 passwordChangeRequired 状态" |
| 问题3（重要）：PasswordChangeService 和 CurrentUser 接口缺少方法签名 | 1.3 节 PasswordChangeService 职责定位补充方法签名：`isChangeRequired/markChangeRequired/clearChangeRequired`；CurrentUser 职责定位补充方法签名：`getUserId/getUsername/getUserType` |
| 问题4（重要）：包 A 实体移至 common-module-impl 的结构变更影响评估不完整——业务模块如何获取用户数据未说明 | 2.2 节依赖规则补充：(a) 用户数据访问途径——通过 common-module-api 中新增 `UserFacade` 门面接口获取；(b) 现有外部引用评估与迁移步骤——列出 6 步迁移计划 |
| 问题5（重要）：SlidingWindowCounter 声明为 public 但两套计数器实例表述雷同，开发者可能误以为共享同一个计数器 | 4.1 节 GlobalRateLimitFilter 段明确标注"**计数器实例独立，互不干扰**"，补充说明 GlobalRateLimitFilter 和 InMemoryRateLimitGuard 各自拥有独立的 `ConcurrentHashMap` 实例 |
| 问题6（一般）：3.4 节"403 FORBIDDEN"与 PasswordChangeCheckFilter 的 PASSWORD_CHANGE_REQUIRED 描述不一致 | 3.4 节将"返回 403 FORBIDDEN"修正为"返回 403 + ErrorCode.PASSWORD_CHANGE_REQUIRED"，同步修正描述 |
| 问题7（一般）：refresh 端点携带 Authorization header 时的服务端行为未完整定义 | 3.1.3 节步骤 1 补充服务端行为描述：JwtAuthenticationFilter 正常处理但 refresh 逻辑不依赖前置认证，Access Token 过期时静默跳过不影响 refresh 流程 |
| 问题8（一般）：login() 锁检查的 IP 维度与 GlobalRateLimitFilter 重复提及，分工不够清晰 | 3.1.1 节步骤 2 将"RateLimitGuard 检查"明确标注为"AuthServiceImpl 内部调用 InMemoryRateLimitGuard.tryAcquire()" |

## 26. 修订说明（v15）

| 审查意见 | 修改措施 |
|---------|---------|
| 问题1（一般）：2.1 节目录结构中 `auth/security/CurrentUser.java`（interface）列在 `common-module-impl` 目录树中，但 2.2 节及迁移步骤要求该接口位于 `common-module-api` 中，两处自相矛盾 | 2.1 节目录结构调整：将 `CurrentUser.java` 从 `common-module-impl/auth/security/` 移至独立的 `common-module-api` 目录树中（`common-module-api/src/main/java/.../auth/CurrentUser.java`）；`common-module-impl/auth/security/` 中仅保留 `CurrentUserImpl.java`（实现类） |

## 27. 修订说明（v16）

| 审查意见 | 修改措施 |
|---------|---------|
| 1. **[重要] ACCOUNT_LOCKED 错误消息与 ErrorCode 表不一致**：3.1.1 节为 IP 维度和用户名维度锁定分别定义了不同的错误消息，但 10.2 节 ErrorCode 表中 ACCOUNT_LOCKED 仅有一条固定消息 | 10.2 节 ErrorCode 表 ACCOUNT_LOCKED 行：消息列改为动态生成描述（"IP 维度锁定→'请 30 分钟后重试'；用户名维度锁定→'请 15 分钟后重试'"），触发场景列补充"在 LoginAttemptTracker 层按锁定维度生成对应消息" |
| 2. **[重要] tokenVersion 比对步骤存在设计歧义**：3.1.3 节步骤 5 从 DB 加载用户（含 tokenVersion），步骤 9 要求验证 tokenVersion 一致，未明确步骤 9 是否重新查询 | 3.1.3 节步骤 5：从 DB 重新加载用户时**暂不加载 tokenVersion**（仅加载 enabled、roles、posts）；步骤 9：明确标注"重新从 DB 加载用户当前 tokenVersion（单独查询，保证强一致性）"，消除复用餐步 5 数据的歧义 |
| 3. **[严重] UserFacade 门面接口缺少完整定义**：未出现在 1.3 节核心抽象一览表、未出现在 2.1 节目录结构、未定义任何方法签名、未说明与 CurrentUser 的职责分工 | 1.3 节核心抽象一览新增 `UserFacade` interface 条目，包含职责定位、方法签名（`findById` / `findByUsername` / `existsById`）、与 CurrentUser 的职责分工说明（CurrentUser = 轻量身份标识会话级访问，UserFacade = 完整业务数据级访问）；2.1 节 `common-module-api` 目录树新增 `UserFacade.java` |
| 4. **[重要] PasswordChangeCheckFilter 白名单路径匹配策略未定义** | 3.3 节 PasswordChangeCheckFilter 行为契约后新增"路径匹配策略"段落：明确使用 `AntPathRequestMatcher` 进行白名单路径匹配，并给出 SecurityConfig 中注册示例和三个白名单匹配器定义 |
| 5. **[重要] MenuUpdateRequest 的 PATCH 语义与 Java record 反序列化存在歧义** | 5.2 节 MenuUpdateRequest 由 Java 17 record 改为传统 POJO 类，补充 PATCH 语义下字段存在性追踪的必要性说明和 `@JsonInclude(NON_NULL)` 控制策略 |
| 6. **[重要] Refresh 端点在禁用用户场景不递增 IP 维度失败计数** | 3.1.3 节步骤 7：在禁用/删除用户判定分支中增加"递增 LoginAttemptTracker IP 维度的失败计数" |
| 7. **[一般] 登录成功后 IP 维度计数器清空前置条件未定义** | 3.1.1 节步骤 8 后新增"IP 维度失败计数器清空的前置假设"段落：说明 NAT/代理共享 IP 场景的局限性、缓解措施（通过 `RequestHelper.getClientIp()` 统一获取 IP、IP 阈值高于用户名维度的缓解设计）；11 节设计决策表新增条目记录此局限性 |
| 8. **[一般] ProfileUpdateRequest.phone 可选性未声明** | 5.2 节 ProfileUpdateRequest record 中 phone 和 email 字段补充注释"可选字段，为空时不更新" |
| 9. **[一般] PasswordChangeRequest.oldPassword 缺少约束** | 5.2 节 PasswordChangeRequest record 中 `oldPassword` 字段补充 `@Size(max = 128)` 约束 |
