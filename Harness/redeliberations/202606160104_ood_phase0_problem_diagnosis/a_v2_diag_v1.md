...
- **§8.2 AI 13 项能力 DTO 字段定义与需求文档 3.4.x 严重不符**：13 项能力中有 11 项（3.4.1 / 3.4.2 / 3.4.3 / 3.4.5 / 3.4.6 / 3.4.7 / 3.4.8 / 3.4.10 / 3.4.11 / 3.4.12 / 部分 3.4.4）存在字段缺失或语义偏差，且 OOD §8.2 行 907 自陈"已与需求文档 3.4.4 / 3.4.9 / 3.4.10 / 3.4.13 对齐"，与实际状态不一致。
...
本报告按"**高 / 中偏高 / 中 / 低 / 极低**"五档优先级组织问题（高：关键遗漏；中偏高：影响面广；中：局部问题；低：表述修正；极低：可选优化），每项均给出具体文档位置、问题描述与诊断依据。**报告停留在定位层，不展开修复方案**——修复者在编码时查阅 Hibernate / Spring Data / Springdoc 文档是正常编码活动，不属于"诊断不完整"。
...
| 能力编号 | 诊断覆盖状态 | 关键问题位置 |
|---------|------------|------------|
| 3.4.1 智能分诊 | 已覆盖 | §2.1 行 36-37 |
| 3.4.2 AI 处方审核 | 已覆盖 | §2.1 行 38-40 |
| 3.4.3 AI 病历生成 | **本轮补全** | §2.1（补） |
| 3.4.4 AI 智能诊断 | 已覆盖（含 `PatientInfo` 偏差） | §2.1 行 41 / §2.2 |
| 3.4.5 AI 智能检查报告 | **本轮补全**（输入 4 项缺失） | §2.1（补） |
| 3.4.6 AI 智能检验报告 | **本轮补全**（输入 2 项缺失 + 结构缺失） | §2.1（补） |
| 3.4.7 AI 影像分析 | **本轮补全**（输入 1 项缺失 + 命名/语义偏差） | §2.1（补） |
| 3.4.8 AI 知识库问答 | 已覆盖 | §2.1 行 49-50 |
| 3.4.9 AI 开立检查/检验 | 已覆盖（含 `PatientInfo` 偏差） | §2.1 行 51 / §2.2 |
| 3.4.10 AI 辅助开方 | **本轮补全**（输出 `PrescriptionDraft` 关联标记） | §2.1（补） |
| 3.4.11 AI 执行顺序推荐 | **本轮补全**（整段遗漏） | §2.1（补） |
| 3.4.12 AI 医生排班 | 已覆盖 | §2.1 行 52-53 |
| 3.4.13 AI 综合讨论结论 | 已覆盖（含 `PatientInfo` 偏差） | §2.2 / §2.4 |

**覆盖率修正值**：v1 覆盖率约 70%（覆盖 9 项），v2 修正为 **100%**（13 项全部覆盖，其中 11 项存在不符点，2 项无不符点）。
...
| **3.4.9 开立检查/检验输出** | 需求 §3.4.9 行 979 / OOD §8.2 行 1022-1026 `ExaminationRecommendResponse` | 缺 `disclaimer_required`（固定 true） | 1 项字段缺失 |
| **3.4.10 AI 辅助开方输出 `PrescriptionDraft`** | 需求 §3.4.10 行 993 / OOD §8.2 行 1057-1058 `PrescriptionDraft` | `prescriptionDraft.drugs` 复用 3.4.2 的 `PrescriptionDrug` 类型（OOD §8.2 行 928-932），字段集偏差（`drug_id`/`route` 缺失 + `dosage`/`days` 语义偏差）已在 §2.1 行 38 标记，但 **3.4.10 输出契约未关联声明**，修复者可能只改 3.4.2 入口而忽略 3.4.10 输出（参见 §3 跨章节一致性观察 4） | 输出契约类型复用未关联标记 |
...
- **修复连锁变更（5 项副作用）**：
  ...
  3. **前端 UI 分支**：已基于 OOD 当前 DTO 编写的前端占位 UI 需调整字段引用（如 `TriageResponse.recommendedDept` → `recommended_departments` 数组结构；3.4.7 输入 `modality` → `model_id` 完全替换）；
  4. **OpenAPI 规范追溯**：§8.3 行 1130 的 springdoc-openapi 配置生成的 OpenAPI schema 与 snake_case 命名约定（见 2.7）需协同调整；
  5. **需求文档 vs OOD 优先级**：若诊断建议全部回退到需求契约，修复者需面对"13 项 AI 能力全部按需求字段集实现"的工作量，与 Phase 0 "骨架" 定位可能冲突——是否仅对齐"骨架必备"的能力（3.4.1 / 3.4.2 / 3.4.3 / 3.4.12）而其他能力保留 Mock 占位，需明确（见 §4 修复指引方向分级建议）。
...
**回收对象 vs 保留对象清单**（修复指引）：
- **保留**：实体类骨架（含主键 + 基本字段，不含 JPA 关联映射）、`DataPermissionEvaluator` 扩展点接口本身（不含数据范围枚举冻结）、`AiService` 接口骨架 + `MockAiService` 占位实现（不含装配策略表）
- **回收**：完整 JPA 关联映射（`@JoinTable` / `@JoinColumn` / `fetch` / `cascade` / `orphanRemoval` 显式声明）、数据范围枚举冻结约定、`@ConditionalOnProperty` 与 `@ConditionalOnMissingBean` 组合策略表、跨模块事件模式示例、ApiClient/AuthStore 完整实现、完整 springdoc-openapi 生产级配置、TypeScript 类型同步迁移路径
- **回收时同步给出 Phase 1 启动时需要重新冻结的内容清单**：完整 JPA 关联映射约定、数据范围枚举值、`MockAiService` 与 `FallbackAiService` 装配优先级、跨模块事件模式选型
...
### 【低优先级】2.8 §4.5 SecurityConfigPhase0 中 `requestMatchers("/api/ping").permitAll()` 与 `anyRequest().permitAll()` 冗余
...
### 【低优先级】2.9 §4.5 Phase 1 profile 切换描述冗余
...
4. **§2.1 中 3.4.2 / 3.4.10 `PrescriptionDrug` 类型复用关联**（与 §2.1 行 38 / §2.1 新增 3.4.10 行 联动）：
   ...
   - 字段集偏差（`drug_id` / `route` 缺失 + `dosage` → `dose` / `days` → `duration` 语义偏差）已在 §2.1 行 38 标记，但未在 3.4.10 行关联声明；
...
修复者在收到本报告后，建议按以下优先级处理（**五档体系：高（关键遗漏）→ 中偏高（影响面广）→ 中（局部问题）→ 低（表述修正）→ 极低（可选优化）**）：
...
1. **【高优先级】首先解决 §8.2 AI 13 项 DTO 与需求 3.4.x 的对齐问题（2.1）**：这是影响最广泛的问题，13 项 AI 能力的 DTO 字段定义决定了 Phase 2-5 全部 AI 能力的契约形态。建议在编码 `MockAiService` 与 `AiService` 接口前先完成字段对齐。
   - **Phase 0 必须对齐的能力**（建议优先对齐核心必填字段，影响 Phase 0 前端独立开发）：
     ...
     - **3.4.12 AI 医生排班**：`constraints` 嵌套结构 + `doctor_pool` 数组 + `historical_schedule_ref` 输入 + `workload_score`（0-100）/ `summary` / `conflicts` 输出；
   - **Phase 1+ 启动前冻结的能力**（保留 Mock 占位即可，待对应阶段启动前再细化字段）：
     - **3.4.4 / 3.4.9 / 3.4.13**：仅需修正 `PatientInfo.allergies` → `allergy_history` 字段语义（已在 §2.2 标记），其余主结构（`ConditionInfo` / `DiagnosisResponse` / `ExamResultSummary`）已对齐；
     ...
     - **3.4.5 / 3.4.6 / 3.4.7 / 3.4.8 / 3.4.11**：核心必填字段在 Phase 1+ 启动前冻结时补全，Phase 0 保留 Mock 占位即可。
...
2. **【高优先级】其次处理 Phase 0 范围越界问题（2.3）**：识别路线图 §0.2 "骨架必备" 与 §0.4 "明确不包含" 的边界，按 §2.3 "回收对象 vs 保留对象清单"回收 OOD 中超出"骨架必备"粒度的内容，降低 Phase 0 工期压力。**精确化边界以避免误删 Phase 0 必须保留的实体骨架**：
   - 保留：实体类骨架（含主键 + 基本字段）、`DataPermissionEvaluator` 扩展点接口本身、`AiService` 接口骨架 + `MockAiService` 占位实现；
   - 回收：完整 JPA 关联映射、数据范围枚举冻结、`MockAiService` / `FallbackAiService` 装配优先级、跨模块事件模式示例、ApiClient/AuthStore 完整实现、完整 springdoc-openapi 生产级配置、TypeScript 类型同步迁移路径。

3. **【高优先级】同步修订 §8.2 行 907 的对齐范围声明（2.4）**：与 2.1 的实际状态保持一致。

4. **【高优先级】修复 `PatientInfo` 字段语义（2.2）**：与 2.1 同步处理（影响 5 项 AI 能力的 `PatientInfo` 共用）。
