...
> 诊断时间：2026-06-16
> 报告版本：v3（第 3 轮迭代，针对组件B第二轮质询反馈修订）
...
| 质询意见 | 回应 |
|---------|------|
| **A1【高】3.4.9 `ExaminationRecommendResponse.disclaimerRequired` 误判为缺失**：OOD §8.2 行 1022-1025 实际已定义 `Boolean disclaimerRequired` 字段，诊断报告 §2.1 / §2.4 / §1 概述 / §1.1 覆盖率表中四处错误地将其归入"字段缺失"，属事实判断错误。 | 已修正。§2.1 不符清单移除 3.4.9 行（`ExaminationRecommendResponse` 行 1022-1025 实际定义 `Boolean disclaimerRequired` 字段，与需求 §3.4.9 行 979 对齐）；§2.4 移除第 3 项"`ExaminationRecommendResponse`（对应 3.4.9）缺 `disclaimer_required`"；§1 概述"11 项"修正为"12 项（11 项全字段不符 + 1 项部分不符 [3.4.4]）"，从列表中移除 3.4.9；§1.1 覆盖率表 3.4.9 行更新关键问题位置为"§2.2"（与 `PatientInfo` 偏差合并处理），覆盖率修正值仍为 100%。 |
| **B1【中】§1.1 覆盖率 100% 声明与 §2.1 实际未为 3.4.13 列出独立字段偏差行不一致**：§1.1 覆盖率表将 3.4.13 标为"已覆盖（含 `PatientInfo` 偏差）"指向 §2.2 / §2.4，但二者均未真正核查 3.4.13 自身字段约束（`discussion_text` ≥ 50 字符、`summary` 项数 0-5 项且每项 ≤ 100 字符、`encounter_id` 必填及特殊校验规则）。 | 已修正。§2.1 不符清单新增 3.4.13 行（输入：`condition.chief_complaint` / `patient_info.patient_id` / `encounter_id` 三者任一缺失的必填校验规则缺失 + `encounter_id` 必填语义未在 OOD 字段注释中标注；输出：`discussion_text` 字符数约束（≥ 50，推荐 ≥ 100）未声明 + `summary` 项数（0-5）与每项字符数（≤ 100）约束未声明 + `DISCUSS_AI_OUTPUT_INCOMPLETE` 错误码未定义）；§1.1 覆盖率表 3.4.13 行更新关键问题位置为"§2.1（补）/ §2.2"；§1 概述将 3.4.13 列入"11 项全字段不符"清单。 |
| **B2【中】§4 修复指引优先级标签与 §2 章节标题优先级标签不一致**：§4 第 2 / 3 项将 §2.3 / §2.4 标为【高优先级】，§4 第 7 项聚合 §2.5 / §2.8 / §2.9 为【中优先级】，但 §2.3 / §2.4 章节标题本身为【中优先级】，§2.8 / §2.9 标题为【低优先级】。 | 已修正。§2.3 章节标题优先级从"中优先级"调整为"高优先级"；§2.4 章节标题优先级从"中优先级"调整为"高优先级"；§2.8 章节标题优先级从"低优先级"调整为"中优先级"；§2.9 章节标题优先级从"低优先级"调整为"中优先级"。§4 修复指引方向优先级标签作为权威，§2 各章节标题已与其对齐。 |
| **B3【低】五档优先级体系声明的"极低"档实际未在 §2 中出现**：§1 与 §4 均声明"高 / 中偏高 / 中 / 低 / 极低"五档，但 §2 全部 14 个子章节中无任何标注为【极低】，实际仅用 4 档。 | 已修正。§1 概述与 §4 修复指引方向的优先级体系声明由"高 / 中偏高 / 中 / 低 / 极低"五档调整为"高 / 中偏高 / 中 / 低"四档（删除未使用的"极低"档）。 |
| **C1【中】§2.2 未纳入 OOD `PatientInfo.allergyDetails` 结构化字段**：OOD §8.2 行 941 实际定义 `List<AllergyDetail> allergyDetails`（含 `allergen` / `reactionType` / `severity` / `occurredAt` 四子字段，显式声明"缺省时回退到 allergies"），诊断报告 §2.2 仅讨论 `allergies: List<String>` vs `allergy_history: string` 偏差，未涉及结构化字段映射规则。 | 已修正。§2.2 新增"结构化字段未对齐（`allergyDetails`）"段，明确 OOD §8.2 行 941 定义 `List<AllergyDetail> allergyDetails`（含 4 子字段）与"缺省时回退到 `allergies`"的显式声明，与需求 3.4.2 / 3.4.4 / 3.4.9 / 3.4.10 / 3.4.13 五项 AI 契约字段集不一致（契约字段集未定义 `allergyDetails`），且路线图 §1.2 行 86 才要求"健康档案基础版（含过敏详情结构化子字段 CRUD）"——属 Phase 1+ 范围，Phase 0 不应定义；§4 第 1 项"Phase 1+ 启动前冻结的能力"显式标注回收 `allergyDetails` 字段；§3 第 6 项新增"§2.2 `PatientInfo` 与 §3.4 Mock 占位规则的耦合"联动分析，说明 `allergyDetails` 在 Mock 阶段按行 628"嵌套 DTO：递归填充"规则会触发 `AllergyDetail` 4 子字段占位填充，与"缺省时回退"语义冲突。 |
| **C2【中】OOD `PatientInfo.name` 字段超出需求未提及**：OOD §8.2 行 936 `PatientInfo` 包含 `String name`，但需求 §3.4.x 各能力的 `patient_info` 契约均未定义 `name`，诊断报告 §2.2 未提及"OOD 超出需求字段"维度。 | 已修正。§2.2 新增"超出需求的字段（`name`）"段，明确 OOD §8.2 行 936 的 `String name`（患者姓名）字段在需求 3.4.2 / 3.4.4 / 3.4.9 / 3.4.10 / 3.4.13 五项 AI 能力的 `patient_info` 契约字段集中均未定义，对 AI 推理无直接用途，且按 §3.4 Mock 占位规则行 629 的"无任何可空标记的字段视为必填"原则成为必填字段，与"骨架必备"路线图 §0.2 行 36-42 的粒度不符；§4 第 4 项"修复 `PatientInfo` 字段语义"显式列出"回收 `name` 字段（超出需求 AI 契约字段集）"。 |
| **C3【低】§4 第 1 项将 3.4.12 列为"Phase 0 必须对齐"缺乏路线图依据**：路线图 §0 将 3.4.1 / 3.4.2 / 3.4.3 列为"核心三件套"，未将 3.4.12（AI 医生排班）纳入 Phase 0 优先序列。 | 已修正。§4 第 1 项"Phase 0 必须对齐的能力"列表移除 3.4.12（AI 医生排班），仅保留 3.4.1 / 3.4.2 / 3.4.3 三项核心必填字段对齐；3.4.12 与 3.4.5 / 3.4.6 / 3.4.7 / 3.4.8 / 3.4.11 一同归入"Phase 1+ 启动前冻结的能力"清单（依据路线图 §0.2 行 50 推荐补齐项"AI 能力模块 Mock 占位（可跨阶段持续完善，不阻塞 Phase 0 骨架验收）"覆盖范围）。 |
| **D1【低】§2.3 "回收 vs 保留"清单中 JPA 关联映射边界不明确**：未明确关联表 schema（`user_role` 等）是否需在 Phase 0 迁移脚本中创建；`@ManyToMany` 等关联注解若不保留，User-Role 之间如何表示关联未说明。 | 已修正。§2.3"回收对象 vs 保留对象清单"扩展为：(a) 保留对象新增"实体类骨架（含主键 + 基本字段 + 简单的 `List<Long>` ID 引用字段如 `roleIds` / `postIds`，**不**使用 `@ManyToMany` 注解）"——明确 User-Role 关联的骨架表示；(b) 回收对象新增"**关联表 schema 本身**（`user_role` / `user_post` / `post_function` 三张表）"——因 `@ManyToMany` 回收后无 JPA 关联注解引用这些表，Phase 0 迁移脚本不应创建这些表；(c) 回收时同步给出 Phase 1+ 启动时需要重新冻结的内容清单新增"完整 JPA 关联映射约定（含关联表 schema 迁移）"。 |
| **D2【低】§2.10 第 3 项对 OOD 设计意图的解读偏差**：OOD §3.4 行 634 明确声明 FallbackAiService"作为装饰器**始终注册为 Bean**"，"始终注册"语义为无条件注册（无 `@ConditionalOnProperty`），诊断报告将其列为"未明示 `@ConditionalOnProperty`"属解读偏差，可能误导执行者误增条件注解。 | 已修正。§2.10 第 3 项修订为"OOD 已明示 FallbackAiService **始终注册为 Bean**（语义为无条件注册，无 `@ConditionalOnProperty` 注解）——此为 OOD 显式声明，修复者按 OOD 实现即可，不需要额外添加条件注解"，明确 OOD 设计意图；同时保留 §2.10 第 1 / 2 项的实际缺口标注（`@Service` / `implements AiService` / `@Primary` 注解未显式声明）；影响范围段落删除"可能因 §3.4 行 634 的'始终注册'已显式声明而正确省略 `@ConditionalOnProperty`"的冗余表述。 |
| **D3【低】§2.1 修复连锁副作用第 3 项"前端 UI 分支"与 Phase 0 实际状态不符**：Phase 0 为"最小化骨架"，路线图 §0.2 验收仅要求"三端前端可一键启动到占位首页"，前端尚未基于具体 AI DTO 编写实际 UI，该副作用提示对 Phase 0 阶段不构成影响。 | 已修正。§2.1 修复连锁变更副作用列表移除"前端 UI 分支"项（"已基于 OOD 当前 DTO 编写的前端占位 UI 需调整字段引用"），原因：路线图 §0.2 行 53 验收标准为"三端前端可一键启动到占位首页"，Phase 0 前端尚未基于具体 AI DTO 编写实际 UI，该副作用对 Phase 0 阶段不构成影响（Phase 1+ 启动 AI 能力前端实现时再考虑）；副作用清单由 5 项精简为 4 项（Mock 实现层重写 / TypeScript 类型同步 / OpenAPI 规范追溯 / 需求 vs OOD 优先级分级）。 |
...
| 质询意见 | 回应 |
|---------|------|
| **1.【关键遗漏】3.4.3 AI 病历生成 整段遗漏**：需补输入 4 项缺失 + 输出 8 字段退化为单字符串的结构偏差对照表；§2.4 补充 3.4.3 实际 OOD 定义与"对齐"承诺的偏差；§4 修复指引方向显式纳入 3.4.3，并指出"流式/非流式架构选型未在 OOD §8.2 中预留"是 Phase 2+ 流式输出的阻塞点。 | 已采纳。§2.1 不符清单新增 3.4.3 行（输入 4 项缺失：缺 `patient_id`（必填）/ `encounter_id`（可选）/ `stream`（bool，可选）/ `dialogue_text` 字符数 50-10000 约束 + 输出 8 字段结构化 vs OOD `structuredRecord: String` 单字符串退化的整体结构不符）；§2.4 第 4 项补充 3.4.3 实际 OOD 定义与"对齐"承诺的偏差（OOD §8.2 行 954 `MedicalRecordGenRequest` 仅 1 字段，与 §8.2 行 907 的"已对齐"承诺产生未声明的偏差）；§4 第 1 项"Phase 0 必须对齐的能力"显式纳入 3.4.3，并明确"流式/非流式架构选型必须在 Phase 0 中预留（否则 Phase 2+ 流式输出需重构响应通道）"。 |
| **2.【关键遗漏】3.4.11 AI 执行顺序推荐 整段遗漏**：需补输入 3 项缺失（`task_items` 子字段 4 项 + `context` + `task_role`）与输出 3 项缺失（`recommended_time` / `summary` / `disclaimer_required`），标注 P1/P2/P3 三档优先级 vs OOD `Integer priority 1-5` 的语义错位；§3 跨章节一致性观察补充：3.4.11 与 3.4.4 / 3.4.5 / 3.4.6 / 3.4.12 在"数值语义风格"上不一致，需在 Phase 1+ 落实时统一约束。 | 已采纳。§2.1 不符清单新增 3.4.11 行（输入 5 项缺失：`task_items` 子字段缺 4 项 + `context` 整体缺失 + `task_role`（必填）整体缺失；输出 4 项缺失：`recommended_time` / `reason`（在 `execution_order` 每项中）+ `summary` / `disclaimer_required`（顶层）+ 优先级 P1/P2/P3 vs OOD `Integer priority 1-5` 语义错位）；§3 第 5 项新增"§8.2 数值语义风格不一致"观察，明确 3.4.11 `priority` 枚举 vs 3.4.12 `workload_score` 0-100 vs 3.4.4 / 3.4.5 / 3.4.6 / 3.4.13 `confidence` 0-100 的选型差异，并建议在 §8.2 补充"数值语义风格约定"段。 |
| **3.【关键遗漏】3.4.5 / 3.4.6 / 3.4.7 输入契约字段遗漏不完整**：3.4.5 仅标"缺 `exam_items`"，遗漏 `exam_id` / `exam_type` / `patient_id` 3 项必填字段；3.4.6 仅标"`rawData: String` vs `lab_items: array`"，遗漏 `lab_id` / `patient_id` 2 项必填字段；3.4.7 仅标"`modality` 代替 `model_id`"，遗漏 `patient_id` 必填字段与 `imageUrl` vs `image_ref` 命名偏差。 | 已采纳。§2.1 不符清单 3.4.5 行补全：缺 `exam_id`（必填）/ `exam_type`（必填）/ `patient_id`（必填）+ 可选 `exam_items`（array，5 子字段）+ `rawData: String` vs `raw_data_ref: String` 命名偏差；3.4.6 行补全：缺 `lab_id`（必填）/ `patient_id`（必填）+ `lab_items` 整体结构（每项含 5 子字段）未引入；3.4.7 行补全：缺 `patient_id`（必填）+ `modality` vs `model_id` 语义完全错配细化（"model_id 是后端岗位推断的模型标识，modality 是模态枚举 CT/MRI/X-Ray，两者属性类别不同"）+ `imageUrl` vs `image_ref` 命名偏差（"url 表示可下载地址，ref 表示任意数据引用"）。 |
| **4.【关键遗漏】3.4.10 输出 `PrescriptionDraft` 字段语义偏差未关联标记**：3.4.10 输出 `prescriptionDraft.drugs` 复用 3.4.2 的 `PrescriptionDrug` 类型，字段集偏差已在 §2.1 行 38 标记，但未在 3.4.10 行关联声明。 | 已采纳。§2.1 不符清单新增 3.4.10 行（"3.4.10 AI 辅助开方输出 `PrescriptionDraft`"），显式标注"prescriptionDraft.drugs 复用 3.4.2 的 `PrescriptionDrug` 类型（OOD §8.2 行 928-932），字段集偏差已在 §2.1 行 38 标记，但 3.4.10 输出契约未关联声明"；§3 第 4 项新增"§2.1 中 3.4.2 / 3.4.10 `PrescriptionDrug` 类型复用关联"联动分析，明确"修复时需同步处理，若只改 3.4.2 入口而忽略 3.4.10 输出，复用同一类型的字段偏差会延续到 3.4.10 的 Mock 实现与前端 UI 渲染"。 |
| **5.【优先级排序】§2.6 / §2.7 优先级上调为"中偏高"；§2.8 / §2.9 优先级下调为"低"；§4 修复指引方向第 5 项将 §2.7 单列为"DTO 字段命名约定（与 §2.1 协同处理，修复成本低）"。** | 已采纳。§2.6（Mock 占位规则语义风险）优先级标记从"中"调整为"中偏高"；§2.7（DTO 字段命名风格）优先级标记从"中"调整为"中偏高"，并补充"全链路契约错位"影响范围与"单一 `@JsonNaming` 配置即可"修复成本说明；§2.8 / §2.9 优先级标记从"中"调整为"低"；§4 修复指引方向第 5 项将 §2.7 单列为"DTO 字段命名约定（与 §2.1 协同处理，修复成本低）"，第 6 项为 §2.6 Mock 占位规则。 |
| **6.【范围越界复核】§3.3 完整 JPA 关系映射越界判断需复核**：引入路线图 §0.2 vs §0.4 对照表，明确骨架必备的实体定义粒度。 | 已采纳。§2.3 引入"路线图 §0.2 骨架必备 vs OOD 定义粒度对照表"，逐项列出 7 处 OOD 定义与路线图 §0.2 要求的对照关系，明确"实体类 + 主键 + 基本字段就位"vs"含完整 JPA 关联映射约定"的边界；§2.3 补充"回收对象 vs 保留对象清单"（保留实体类骨架 / `DataPermissionEvaluator` 扩展点接口本身 / `AiService` 接口骨架 + `MockAiService` 占位；回收完整 JPA 关联映射 / 数据范围枚举冻结 / 装配优先级 / 跨模块事件模式 / ApiClient/AuthStore / 完整 springdoc 配置 / TypeScript 类型同步迁移路径）；§4 第 2 项精确化"避免误删 Phase 0 必须保留的实体骨架"。 |
| **7.【副作用提示】§2.1 / §2.2 / §2.3 修复方案潜在副作用提示缺失**：DTO 字段对齐会触发 5 项连锁变更；Phase 0 范围回收影响 Phase 1 启动；§4 修复指引方向区分"Phase 0 必须对齐的能力"与"Phase 1+ 启动前冻结的能力"。 | 已采纳。§2.1 影响范围补充 5 项连锁变更副作用（Mock 实现层重写 / TypeScript 类型同步 / 前端 UI 分支调整 / OpenAPI 规范追溯 / 需求 vs OOD 优先级分级）；§2.2 影响范围保留；§2.3 影响范围补充 3 项副作用（Phase 1 权限模型落地 / Phase 1 统一认证 / "骨架可演进"目标冲突）；§4 修复指引方向第 1 项区分"Phase 0 必须对齐的能力"（3.4.1 / 3.4.2 / 3.4.3 / 3.4.12 核心必填字段）与"Phase 1+ 启动前冻结的能力"（其他能力保留 Mock 占位即可）。 |
| **8.【证据格式】§1.4（3.4.10 关联缺失）证据格式与 §1.1 / §1.2 / §1.3 不一致**：补充 OOD `PrescriptionDraft` / `prescriptionDraft.drugs` 的具体行号。 | 已采纳。§2.1 不符清单中 3.4.2 / 3.4.10 / 3.4.11 / 3.4.3 / 3.4.5 / 3.4.6 / 3.4.7 等所有新增/扩充行的"需求条款"列与"OOD 位置"列采用统一的"需求 §X 行 Y / OOD §X 行 Z"双侧行号对照格式（如 3.4.10 行：需求 §3.4.10 行 993 / OOD §8.2 行 1057-1058 `PrescriptionDraft`）；§2.4 第 4 项引用具体行号"OOD §8.2 行 954 `MedicalRecordGenRequest` 仅 1 个字段"。 |
| **9.【优先级体系】五档扩展未显式声明**：在 §4 / §6 优先级排序开头显式声明五档体系。 | 已采纳。§1 概述补充五档优先级体系声明（"高（关键遗漏）→ 中偏高（影响面广）→ 中（局部问题）→ 低（表述修正）→ 极低（可选优化）"）；§4 修复指引方向开头显式声明五档体系。 |
| **10.【覆盖状态核查】3.4.13 覆盖状态核查遗漏**：§1 增加对 3.4.13 覆盖状态的核查；§7 完成度评估表中明确覆盖率修正值。 | 已采纳。§1.1 新增"13 项 AI 能力诊断覆盖率核查"表格，逐项列出 13 项 AI 能力的诊断覆盖状态与关键问题位置；覆盖率修正值由 v1 的"约 70%（覆盖 9 项）"修正为 v2 的"100%（13 项全部覆盖，其中 11 项存在不符点，2 项无不符点）"。原 §7 完成度评估表未在 v1 中存在，故在 §1.1 直接呈现覆盖率修正值。 |
| **11.【次要问题合并】§5.1 §3 重复、§5.2 §4 第 5 项归类、§5.3 §5 范围说明精简、§4.3 §2.8 / §2.9 优先级下调合并精简为"次要表述问题汇总"一节；§2.6 枚举风险表述精确化；§2.10 FallbackAiService 表述澄清；§2.5 修复指引补充"联动修改 §7 决策表对比维度"。** | 已采纳。§2.6 枚举风险表述精确化为"Mock 默认填充值为 `EnumType.values()[0]`（即枚举声明顺序的第一个值），若声明顺序与业务中性值不一致（如 HIGH 在前），则 Mock 默认填充高风险/严重值"，并列出各枚举的潜在偏差方向；§2.10 FallbackAiService 表述澄清为"OOD §3.4 行 634 声明 FallbackAiService'作为装饰器始终注册为 Bean'，但未显式给出其 `@Service` 注解、`implements AiService` 实现关系及 `@Primary` 装饰器标注，导致 Spring 类型注入 `List<AiService>` 时是否包含自身依赖 OOD 隐含行为推断"；§2.5 影响范围补充"修复副作用：修订时需联动调整 §7 行 853 决策表的对比维度"；§4 第 7 项"术语与表述修正"包含 §2.5 / §2.8 / §2.9，§2.8 / §2.9 优先级下调为"低"。原 §5 三个次要子问题与原 §4.3（§2.8 / §2.9 优先级下调）已通过本表各项具体回应体现精简后的统一归类，不再单独列出重复章节。 |
