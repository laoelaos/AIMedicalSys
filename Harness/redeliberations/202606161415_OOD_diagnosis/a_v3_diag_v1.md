...
- **建议（含可行方案评估）**：
  ...
  **关于空壳 DTO 与 MockAiService 占位数据约定的冲突分析**：
  ...
  - **子方案 A2（保留 Phase 0 关键字段子集）**：为 Phase 0 保留少量关键字段（如 `TriageRequest.patientId`、`TriageResponse.level` 等真正需要 Mock 填充的字段）作为"Phase 0 Mock 字段子集"，其余字段保持空壳。在 DTO 类中以注释标记哪些字段属于 Phase 0 占位字段。此方案保留了前端数据绑定验证能力，但需要在 DTO 类中维护 Phase 0 字段标记，后续阶段补充字段时需清理这些标记。
  ...
  **结论：推荐方案 A + 子方案 A2。** 保留空壳 DTO 类声明以满足编译期约束，同时为 Phase 0 保留 `TriageRequest.patientId`、`TriageResponse.level`、`TriageRequest.symptoms` 等核心字段作为 Mock 占位字段子集，以注释 `// @phase0-mock-field` 标记。其余字段推迟到对应阶段 OOD 中补齐。这一方案兼顾了编译期约束、路线图范围合规性、前端 Phase 0 验证能力三方面需求。第 623–629 行的 Mock 占位约定文本需同步补充说明：仅对标记为 `@phase0-mock-field` 的字段生效，非标记字段 Mock 返回 null/default。
...
|--------|------|------|---------|-------------|---------|
| P0 | 5 | 偏离路线图 | 严重 | 882–1270 | 先决条件：确认方案 A + 子方案 A2 的空壳 DTO + Phase 0 Mock 字段子集策略。独立可执行，不依赖其他问题修复 |
...
| P1 | 3 | 事实错误 | 中等 | 304, 307 | 修正依赖传播事实描述文本。P1/P0 完成后文档描述需与新的依赖声明保持一致。与问题 1 共同涉及第 304–307 行的文本修订，建议统一排入 P1 一并处理 |
| P2 | 7 | 文档表达 | 轻微 | 272–287 | 重新绘制 ASCII 图，独立可执行。不依赖其他问题 |
| P3 | 4 | 逻辑错误 | 轻微 | 658–662, 645 | 添加 WARN 日志 + @PostConstruct 启动期 ERROR 日志。独立可执行，代码级改动无需等待其他问题 |
| P3 | 6 | 偏离路线图 | 轻微 | 587–593 | 移除 DataScopeType 枚举值，保留接口声明。建议在 P1 完成后做，避免文本修订范围冲突 |

**依赖关系说明**：
- P0 独立，无前置依赖（但需在子方案 A2 确认后再实施 P1 的文档文本修订）
