...
### 问题 4：集成测试模块缺少 h2 运行时依赖

- **所在位置**：第 1496–1548 行（integration/pom.xml 骨架）
- **严重程度**：严重
- **问题描述**：
  integration 模块以 `test` scope 依赖 application 模块。application 模块的 H2 依赖声明为 `runtime` scope（第 1407–1411 行），且 application 的 Spring Boot Maven Plugin 配置了 `<classifier>exec</classifier>`（第 1486–1493 行）以生成普通 JAR 供 integration 依赖。
  但 integration 模块使用 `test` scope 依赖 application 后，application 的 runtime scope 依赖（H2）不会被传递到 integration 的 test classpath 中。`@SpringBootTest` 在启动时会尝试配置 `spring.datasource.url=jdbc:h2:mem:aimedical`，但 classpath 上缺少 H2 驱动，导致 `DataSource` 配置失败。
  虽然第 1484 行通过 classifier exec 解决了 fat JAR 的 transitive 依赖解析问题，但 runtime → test scope 的传递性丢失是另一个独立问题。
- **建议**：
  integration 模块的 POM 中应显式声明 H2 依赖（`test` scope），或 application 模块改用 `compile` scope 声明 H2（不推荐，会影响生产包体积）。推荐方案：integration 模块显式添加 h2 依赖。

---

## 三、逻辑错误（逻辑错误）

### 问题 5：PageQuery sort 参数自定义解析与 Spring Data 原生能力重复

- **所在位置**：第 469 行
- **严重程度**：中等
- **问题描述**：
  PageQuery 的 sort 字段定义为 `List<String>`，描述要求"Controller 或 Service 在组装 Pageable 时逐项转换为 `Sort.Order`"。Spring Data 的 `@PageableDefault` 和 `@SortDefault` 原生支持通过 `?sort=createdAt,desc&sort=id,asc` 格式的多值绑定，直接返回 `Pageable` 对象，无需手动解析。
  设计自定义 PageQuery 并手动解析 sort 参数，相当于在 Spring Data 已原生支持的同一套参数格式之上重建解析逻辑，且引入了额外的校验（格式无效时抛 `BusinessException(PARAM_INVALID)`）。这增加了维护成本和出错概率，且与 Spring Data 的 Sort 绑定机制产生竞争关系。
- **建议**：
  Controller 层直接使用 `@PageableDefault` + `@SortDefault` 接收分页参数，内部转换为 `PageResponse<T>` 统一返回。或保留 PageQuery 但委托 Spring Data 的 `SortHandlerMethodArgumentResolver` 处理 sort 参数绑定，避免手动解析。
...
## 四、偏离路线图 Phase 0 阶段（偏离路线图phase0阶段）

### 问题 7：完整定义 13 项 AI 能力的 DTO 字段级契约，构成模块级接口契约冻结
...
### 问题 8：数据权限扩展点设计（`DataPermissionEvaluator`）超出 Phase 0 骨架范围
...
| 4 | 事实错误 | 严重 | 1496–1548 |
| 5 | 逻辑错误 | 中等 | 469 |
| 6 | 逻辑错误 | 轻微 | 658–662, 645 |
| 7 | 偏离路线图phase0阶段 | 严重 | 882–1270 |
| 8 | 偏离路线图phase0阶段 | 轻微 | 587–593 |
