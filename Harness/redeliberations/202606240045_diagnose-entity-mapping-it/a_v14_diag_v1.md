...
| M10 | Role | `users` `@ManyToMany(mappedBy = "roles")` | Role.java:36-37, user_role 关联表 | M4 已覆盖正向（User→roles），M10 是 mappedBy 端由 Hibernate 自动维护，无需单独测试 |
...
- **M10（Role.users mappedBy 端）覆盖说明**：映射点 M10 标注为 `Role.users @ManyToMany(mappedBy = "roles")`。在 JPA 双向 `@ManyToMany` 中，`mappedBy` 端由 Hibernate 根据 owning 端的变更自动维护关联表记录。M4（`User.roles`，使用 `@JoinTable` 的 owning 端）已通过 `user_shouldMapManyToManyWithRoles` 测试覆盖正向映射——该测试从 User 侧持久化 roles 关系并验证关联表数据。M10 作为 mappedBy 端无需编写独立测试，Hibernate 规范保证其与 owning 端共享同一关联表记录。如需额外验证，可在 `user_shouldMapManyToManyWithRoles` 中扩充断言——从 Role 侧反向获取 users 并验证其 size 和内容，但当前方案认为 M4 已充分覆盖，M10 跳过理由充分。
...
- **表锁风险**：ALTER TABLE MODIFY COLUMN 在 MySQL 中可能引发表锁或重建表。`sys_user` 表作为核心用户表，在典型业务系统中数据量可能较大。建议在低峰期执行该 DDL 变更，或使用 pt-online-schema-change 等工具减少锁时间。此分析与 Issue 3 的 ALTER TABLE 风险评估标准一致。
...
**操作顺序建议**：①预检确认脏数据量 → ②业务决策选择清理方案 → ⚠ 执行数据清理前，必须确认 Issue 4 的 enabled 清理已完成（顺序：先 Issue 4 enabled 清理 → 再 Issue 2 数据清理，详见下节"与 Issue 4 清理策略的交叉数据冲突"）→ ③执行数据清理 → ④在变更窗口执行 DDL 变更（添加 NOT NULL）→ ⑤验证约束生效（尝试插入 NULL 应被拒绝）
...
**(B) DEFAULT 行为验证**
```sql
-- 插入不指定 deleted 列的行，验证 DEFAULT 0 是否生效
INSERT INTO sys_user (username, password, user_type)
VALUES ('test_default_deleted', 'pwd', 'ADMIN');

SELECT id, username, deleted FROM sys_user WHERE username = 'test_default_deleted';
-- 预期结果：deleted = 0（DEFAULT 生效），而非 NULL
```

**(C) @SQLRestriction 后置验证**
```sql
-- 确认 deleted = 0 的记录可被正常返回
SELECT id, username, deleted FROM sys_user WHERE deleted = 0;
-- 预期结果：所有 deleted = 0 的记录正常可见

-- 确认清理完成：所有 NULL 已被 UPDATE 为 0，再无隐藏记录（需先执行 UPDATE 清理使 deleted IS NULL 不复存在，但仍应确认）
SELECT COUNT(*) AS still_null FROM sys_user WHERE deleted IS NULL;
-- 预期结果：0（所有 NULL 已被清理，再无隐藏记录）
```

此外，应在应用代码层面执行一次 Hibernate 查询（如通过 RestTemplate 调用实际 API 或运行 EntityMappingIT 中的集成测试），确认 `@SQLRestriction("deleted = false")` 过滤后查询结果符合预期——仅返回 `deleted = 0` 的记录，`deleted IS NULL` 的记录不再出现。这是 Issue 3 被提升至 P1 的核心业务影响验证维度，不可省略。
...
回滚包含两个子步骤：**数据回滚**（恢复 deleted 列原始值）和 **约束回滚**（撤销 ALTER TABLE 添加的 NOT NULL 约束），必须按此顺序执行。
...
回滚步骤 A——数据回滚（从备份恢复 deleted 原始值，全部 16 张表逐一执行）：
```sql
...
回滚步骤 B——约束回滚（撤销 ALTER TABLE MODIFY COLUMN 添加的 NOT NULL 约束，恢复至迁移前允许 NULL 的列定义，全部 16 张表逐一执行）：
```sql
ALTER TABLE sys_user        MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE sys_role        MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE sys_post        MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE sys_function    MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE sys_dict_type   MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE sys_dict_data   MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE patient_profile MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE doctor_profile  MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE admin_profile   MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE health_profile  MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE allergy_history MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE chronic_disease MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE family_history  MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE surgery_history MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE medication_history MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
ALTER TABLE sys_token       MODIFY COLUMN `deleted` TINYINT(1) DEFAULT 0 COMMENT '逻辑删除';
```
...
| B. @ColumnDefault 注解 | 在字段上添加 `@ColumnDefault("1")` | Hibernate 生成 DDL 时会包含 DEFAULT；语义显式 | 不改变 Java 端行为——通过代码创建实体后不 setEnabled 仍然为 null，仅影响 DDL 生成 |
...
推荐采用**两阶段执行策略**，将代码修复与数据清理分离，避免混合排序导致的时序冲突：

**阶段一：代码修复**（无运行时风险，可按任意顺序执行，推荐如下）
1. Issue 2 代码注解修复：在 User.java:28 添加 `@Column(nullable = false)`（仅改 Java 源码，不影响运行中数据库）
2. Issue 3 schema.sql 修改：在 16 张表的 deleted 列定义中添加 NOT NULL（仅改源码文件，不影响运行中数据库）
3. Issue 4 Java 默认值修复：在 User/Role/Post/Function 的 enabled/visible 字段添加 `= true` 初始值（仅改 Java 源码）
4. Issue 1 新增集成测试：新增 EntityMappingIT 测试方法验证上述修复后的映射正确性

**阶段二：数据清理**（必须在同一变更窗口按以下顺序执行，不可调换）
1. Issue 4 数据清理：`UPDATE sys_user SET enabled = 1 WHERE enabled IS NULL`（及 Role/Post/Function 同理）
2. Issue 2 数据清理（方案 B 场景）：`UPDATE sys_user SET enabled = 0 WHERE password IS NULL`（将无密码账号禁用——必须在 Issue 4 清理之后执行，否则无密码且 enabled=NULL 的记录会被误启用）
3. Issue 3 数据清理：`UPDATE ... SET deleted = 0 WHERE deleted IS NULL`（16 张表逐一执行）
4. Issue 3 DDL 变更：ALTER TABLE ... MODIFY COLUMN deleted TINYINT(1) NOT NULL DEFAULT 0（16 张表）
5. Issue 2 DDL 变更：ALTER TABLE sys_user MODIFY COLUMN password VARCHAR(128) NOT NULL

说明：Issue 2 的代码修复（添加 @Column 注解）与数据清理（处理已有 NULL password）属于两个独立维度，不应被"线性顺序"隐式绑定。阶段二第 1、2 步的顺序是安全的——先 Issue 4 将所有 NULL enabled 置为启用，再 Issue 2 将无密码账号禁用，最终语义为"无密码账号被禁用"。若顺序颠倒（先 Issue 2 再 Issue 4），无密码且无 enabled 值的记录会被置为启用，形成安全漏洞。Issue 3 ALTER TABLE 放在最后，因为涉及 16 张表且需在数据清理完成后执行，避免因脏数据导致 ALTER 失败。
...
| 质询意见 | 回应 |
|---------|------|
| 1. [严重] 建议修复顺序与数据清理时序依赖矛盾——第804行推荐执行顺序"Issue 2 → Issue 3 → Issue 4 → Issue 1"，但交叉数据冲突分析明确要求Issue 4的清理SQL必须在Issue 2的方案B之前执行，否则产生安全漏洞。报告将"代码修复"与"数据清理"混合在同一线性顺序中，未区分两个维度。 | **接受**。已将推荐顺序重构为两阶段策略：阶段一（代码修复：Issue 2 注解 → Issue 3 schema → Issue 4 默认值 → Issue 1 测试）；阶段二（数据清理：Issue 4 清理 → Issue 2 清理 → Issue 3 清理 → Issue 3 DDL → Issue 2 DDL）。新增「数据清理执行顺序」子章节，显式标注各步的先后依赖及其安全理由。 |
| 2. [中等] Issue 3 回滚方案未还原 ALTER TABLE 约束变更——回滚仅从备份表恢复 deleted 数据值，但 ALTER TABLE MODIFY COLUMN 已施加的 NOT NULL 约束未被撤销。回滚后列仍为 NOT NULL，与回滚前状态不一致。 | **接受**。在 Issue 3 回滚方案中新增「回滚步骤 B——约束回滚」子章节，逐一列出全部 16 张表的 ALTER TABLE MODIFY COLUMN 移除 NOT NULL 约束的 SQL，恢复至迁移前的列定义（`deleted TINYINT(1) DEFAULT 0`）。明确回滚分两步：先数据回滚（恢复 deleted 值），再约束回滚（撤销 NOT NULL）。 |
| 3. [一般] Issue 3 迁移验证步骤遗漏 DEFAULT 行为和 @SQLRestriction 行为验证——验证仅包含 (A) 确认 NOT NULL 约束和 (B) 插入 NULL 被拒绝。缺少 DEFAULT 行为验证（插入不指定 deleted 的行验证 DEFAULT 0 生效）和 @SQLRestriction 后置验证（迁移后 deleted=0 正常返回、deleted IS NULL 已被过滤）。 | **接受**。在 Issue 3 步骤 4 验证中补充：(B) DEFAULT 行为验证——插入不指定 deleted 的行并验证其值为 0；(C) @SQLRestriction 后置验证——查询 deleted=0 的记录正常返回、确认不再有 deleted IS NULL 的记录，同时建议在应用代码层面执行 Hibernate 查询确认过滤行为正确。 |
| 4. [一般] Issue 2 的 ALTER TABLE 表锁风险未被纳入副作用分析（与 Issue 3 论证标准不对称）——Issue 2 副作用分析未提及 ALTER TABLE 表锁风险，而 Issue 3 的副作用分析明确提及大表锁问题。 | **接受**。在 Issue 2 副作用分析中新增「表锁风险」条目，说明 ALTER TABLE MODIFY COLUMN 可能引发表锁或重建表，标注 sys_user 表规模和预估影响，并给出与 Issue 3 相同的执行窗口建议（低峰期执行或使用 pt-online-schema-change）。 |
| 5. [一般] M10（Role.users @ManyToMany mappedBy 端）历经12轮仍标注"未覆盖"但未提供处理策略——M10 备注列标注"未覆盖"，@ManyToMany 映射测试策略说明亦未给出处理建议。 | **接受**。采取方案 B——明确论证"M4 已覆盖正向映射（User→roles），M10 作为 mappedBy 端由 Hibernate 自动维护，无需单独测试"并在映射点表备注列更新为跳过理由。同时在「@ManyToMany 映射测试策略说明」中新增「M10 覆盖说明」条目，详细阐述 JPA 双向 @ManyToMany 的 mappedBy 端与 owning 端共享同一关联表的规范依据，及 M4 已充分覆盖的理由。 |
...
| 质询意见 | 回应 |
|---------|------|
| 1. [中等] Issue 2 DDL 变更在建议修复顺序中被标注为"可选"，与 Issue 2 自身修复指引矛盾——Phase 2 第 5 步写为"Issue 2 DDL 变更（可选）"将 ALTER TABLE 标记为可选，但 Issue 2 的修复指引明确要求"将 schema.sql:16 的 DEFAULT NULL 改为 NOT NULL"，且指出"两处改动需同步进行"。标注"可选"会误导执行者跳过必要步骤。 | **接受**。已移除"（可选）"标注，直接写为"Issue 2 DDL 变更：ALTER TABLE sys_user MODIFY COLUMN password VARCHAR(128) NOT NULL"。 |
| 2. [一般] Issue 2 主操作顺序建议未显式纳入与 Issue 4 的交叉数据清理时序依赖——操作顺序建议仅列出"①预检 → ②决策 → ③数据清理 → ④DDL → ⑤验证"五步，未在第③步之前提及与 Issue 4 清理的顺序依赖。仅阅读 Issue 2 章节的执行者可能直接按主操作顺序执行，在第③步运行 Issue 2 方案 B 而遗漏 Issue 4 的前置清理，导致安全漏洞。 | **接受**。在"操作顺序建议"第③步之前增加显式引用："⚠ 执行数据清理前，必须确认 Issue 4 的 enabled 清理已完成（顺序：先 Issue 4 enabled 清理 → 再 Issue 2 数据清理，详见下节"与 Issue 4 清理策略的交叉数据冲突"）"。 |
| 3. [轻微] @ColumnDefault 示例值 `"true"` 与项目 DDL 约定不一致——方案 B 表格中写 `@ColumnDefault("true")`，但项目 schema.sql 中所有 Boolean 字段的 DDL 默认值均使用 `DEFAULT 1`（数字字面量），而非 `DEFAULT true`。使用 `@ColumnDefault("true")` 生成的 DDL 会变成 `DEFAULT true`，与手工维护的 schema.sql 出现格式差异。 | **接受**。已将 `@ColumnDefault("true")` 改为 `@ColumnDefault("1")`，以匹配项目 DDL 中 `DEFAULT 1` 的约定，避免生成不一致的 DDL。 |
| 4. [轻微] Issue 3 的 @SQLRestriction 验证步骤注释存在表述落差——步骤 4(C) 的第二条 SQL 查询注释写为"确认 deleted IS NULL 的记录已被过滤"，但该查询实际验证的是 UPDATE 清理后不再存在 NULL 记录，属于"数据清理结果确认"而非"@SQLRestriction 过滤行为验证"。 | **接受**。已将注释改为"确认清理完成：所有 NULL 已被 UPDATE 为 0，再无隐藏记录（需先执行 UPDATE 清理使 deleted IS NULL 不复存在，但仍应确认）"，将 SQL 级别的验证和 Hibernate 层面的过滤验证在语义上明确区分。 |
