...
- **NOT NULL 约束验证策略**：User.userType 标注了 `@Column(nullable = false)`，测试应通过 persist 一个未设置 userType 的 User 来验证 `ConstraintViolationException`。同理，BaseEntity.deleted 的 NOT NULL 约束可通过 persist 后检查值来验证（因 BaseEntity 已设 `deleted = false` 默认值，不会出现 null）。password 的 NOT NULL 约束验证见 `user_shouldRejectNullPassword()`。
