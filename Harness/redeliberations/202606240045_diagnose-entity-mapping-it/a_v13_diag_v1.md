...
-- 确认 deleted IS NULL 的记录已被过滤（需先通过 UPDATE 清理使 deleted IS NULL 不复存在，但仍应确认）
...
5. Issue 2 DDL 变更（可选）：ALTER TABLE sys_user MODIFY COLUMN password VARCHAR(128) NOT NULL
