...
| 质询意见 | 回应 |
|---------|------|
| [问题1·一般] T3 修复建议采用"或"连接两个选项，未给出推荐选择 | 已修正：T3 修改建议改为直接指向"修正审查范围描述中的测试文件路径"，说明 scope.md 本身未承诺模块归属，偏差源于审查者预期偏差。 |
| [问题2·一般] T17 修复方案缺少技术路线指引 | 已修正：在 T17 分析中新增"推荐技术路线"章节，明确推荐方案 C（抽取 MessageInterpolator 组件，统一注入 GlobalExceptionHandler + RestAuthenticationEntryPoint + RestAccessDeniedHandler），并说明选择理由及排除方案 A/B 的依据。 |
| [问题3·一般] T13 根因分析未辨识 OOD 4.1 节第 433 行与第 444 行的内在文本矛盾 | 已修正：在 T13 分析中补充 OOD 文本模糊性分析（"每 IP 独立加锁" vs "ReentrantLock 保护原子性"的矛盾），将根因归属从"编码与 OOD 不一致"改为"OOD 文本内在模糊 + 编码实现跟随"，并建议修复时同步澄清 OOD 文本。 |
| [问题4·一般] 可操作性维度缺乏系统性风险评估 | 已修正：修复批 3 新增"子任务间干扰评估"（分析 T9/T10/T11/T18 之间的文本冲突与并行可行性）；修复批 4 新增"回滚影响范围"（标注四文件互斥，建议分次提确保单项可独立回滚）。 |
| [问题5·轻微] 测试问题章节标题（T3-T7, T20-T34）与正文分类（T3 归入其他）矛盾 | 已修正：标题从"汇总：测试问题（T3-T7, T20-T34）"改为"汇总：测试相关问题（T4-T7, T20-T34）"，与正文分类结论一致。 |
| [问题6·轻微] 修复批 6 标题"测试增强"未反映 T3 的文档对齐任务 | 已修正：标题从"修复批 6：测试增强"改为"修复批 6：测试增强与文档对齐"；T3 条目描述同步更新为"修正审查范围描述中的测试文件路径"，明确标注"文档对齐任务而非测试增强"。 |
| [问题7·轻微] P2 优先级中 T22 排除标注冗余 | 已修正：删除 P2 优先级行末尾的"（T22 为误报，无需修复）"标注，因 T22 编号不在 T23-T34 范围内，该说明无实质作用。 |

## 修订说明（v8）

| 质询意见 | 回应 |
|---------|------|
| [问题一·一般] T13 根因分类与汇总表之间存在事实矛盾：正文分析明确根因为"OOD 文本模糊性（主因）"，但 OOD 缺陷汇总表未包含 T13，编码缺陷汇总表将 T13 根因简述为"全局锁而非每 IP 独立锁"，未反映 OOD 为主因的事实 | 已修正：OOD 文档缺陷汇总表新增 T13 条目（缺陷类型：文本内在矛盾；位置：`05_ood_phase1_B.md` 4.1 节第433行与第444行间矛盾）；编码缺陷汇总表 T13 根因简述改为"OOD 文本模糊 → 编码跟随其中一种解读（次因：编码未质疑矛盾）"，体现双归属。 |
| [问题二·一般] 修复批次 1-5 遗漏"存量测试同步更新"指引：T1/T9/T11/T15 的行为变更将导致现有测试断言失效或编译失败，修复执行者若未预知此风险，完成后将发现测试中断 | 已修正：修复批 1-5 各条均新增"受影响存量测试"子项，明确列出每个修复涉及的具体测试文件、变更性质（更新断言/移除旧测试/新增场景）及风险点。共涉及 `JwtTokenProviderTest`、`AuthServiceTest`（3 处）、`AuthControllerTest`、`SlidingWindowCounterTest`、`LoginAttemptTrackerTest`、`MenuControllerTest`、`JwtUtilTest` 等文件。 |
| [问题三·轻微] 修复批 6 分组方案缺乏可操作的具体映射：仅给出"5-6 个子任务，每个覆盖 3-5 个测试文件"的泛泛建议，未提供具体的 task-to-group 分配、执行顺序、任务类型区分 | 已修正：将修复批 6 拆分为 8 个具体子任务（A-H），以表格形式提供每个子任务的类型（文档对齐/新增测试/修正测试）、目标文件、功能点数、执行顺序与依赖关系、注意事项。子任务 A 为文档对齐独立项；子任务 B-H 按同文件合并、功能点类型分组，并标注了 T6 对 T2 的依赖及 T25 对 T13 的依赖。 |

## 修订说明（v9）

| 质询意见 | 回应 |
|---------|------|
| [严重] 汇总表OOD缺陷计数与详细表矛盾：总览表显示OOD缺陷计数为3但OOD详细表列出4项（含T13） | 已修正：OOD详细表移除T13（T13仍保留在编码缺陷汇总表），保持总览表计数（OOD=3）与详细表一致。OOD表格下方新增注释说明T13双归属已在编码缺陷汇总表中呈现。 |
| [一般] T13双归属缺乏交叉引用标注 | 已修正：编码缺陷汇总表T13条目末尾追加「↗ 主因为OOD文本模糊性」交叉引用标注。 |
| [一般] T14优先级与P1定义不匹配：T14（JwtUtil.generateToken遗留claims）仅用于测试环境，不影响生产运行时行为 | 已修正：T14从P1移至P2。 |
| [轻微] 修复批6子任务D未标注T20对T17的潜在依赖 | 已修正：子任务D执行顺序与依赖列增加说明——T20建议在T17之后实施，因T17将formatMessage抽取为MessageInterpolator后T20的测试更适合对MessageInterpolatorTest进行。 |

## 修订说明（v10）

| 质询意见 | 回应 |
|---------|------|
| [问题1·一般] 依赖图遗漏 T20→T17 和 T25→T13 两条依赖边：子任务表已标注但依赖图正文 P2 区块仍标注「无交叉依赖，可任意顺序」 | 已修正：P2 依赖图增加 T20→T17 和 T25→T13 两条有向边，P2 开头标注改为「除以下标注依赖外无交叉依赖」，确保依赖图正文与子任务备注完全一致。 |
| [问题2·轻微] T3 修复目标文件未明确指定：子任务 A 目标文件仅标注为「审查范围描述文件」 | 已修正：子任务 A 目标文件明确指定为 `Harness/reviews/202606270204_fix_phase1B_code_review/review_v2_D.md`，备注中说明需修改该文件审查范围描述章节的路径信息。 |
| [问题3·轻微] 修复批 4 未评估 T12 与 T13 的潜在测试资源冲突 | 已修正：经核实 `SecurityConfigPhase1Test` 和 `SecurityConfigPhase1CoexistenceTest` 虽同时引用两组件但仅作为独立 Bean 分别验证，无交叉交互场景。在修复批 4 新增风险说明确认理论交叉影响可忽略。 |
| [问题4·轻微] T6 响应行为验证对 T2 的依赖描述存在模糊性：未说明如何规避执行时测试失败 | 已修正：子任务 B 执行顺序列补充具体指引——响应行为验证建议先用 @Disabled 标注或条件断言 assumeTrue 避免因预期值不符失败，待 T2 实施后再激活。 |
| [补充] 优先级定义边界争议：P1 定义「功能正确性受影响」未直接覆盖「关键测试缺失」 | 已修正：在 P1 优先级列表末尾补充注释说明「关键测试缺失归入 P1」的判定依据——T5/T6/T7 均对应影响功能正确性的关键路径，因此归入 P1。 |

## 修订说明（v11）

| 质询意见 | 回应 |
|---------|------|
| [严重] 问题1：T13根因分类与总览表之间存在结构性矛盾——v9采用"从OOD表移除T13+注释"未根本解决 | 已修正：放弃"移出"方案，改为方案A——OOD缺陷表重新纳入T13并标注双归属，总览表OOD计数调整为"4（含1项双归属，T13）"，编码缺陷计数调整为"10（T13主因已计入OOD侧）"。OOD表注释同步更新为双归属说明。 |
| [一般] 问题2：修复批2标题"OOD文档对齐"严重低估T17的实际工作量（中大型编码重构），且违反OOD文档修改与对应编码修建议分两次提交的原则 | 已修正：将批2拆分为批2a（OOD文档更新：T8+T17-OOD）和批2b（编码实现：T17-编码），后续批次序号相应递推（原批3~6改为批3~6）。 |
| [一般] 问题3：T12的P1优先级归类缺乏说服力——T12仅在启动阶段配置校验路径生效，不涉及运行时业务逻辑，与T14降级逻辑不一致 | 已修正：T12从P1移至P2。P1列表移除T12条目，P2列表新增T12。 |
| [轻微] 问题4：T9对OOD 3.1.4 Step 4的引用存在断章取义风险——"无论是否携带refreshToken"修饰的是参数存在性而非token有效性 | 已修正：删除OOD 3.1.4 Step 4引用行，改为以OOD 4.8节的可选字段设计作为主要支撑论据。 |
| [轻微] 问题5：修复批4缺少OOD文档更新步骤——T12和T13的修复建议均涉及OOD文档更新但批4被归类为纯编码修复批次 | 已修正：在批4的T12和T13条目中各新增OOD文档更新子项，与编码实现子项并列。提交建议从"四次独立提交"调整为"六个子项按文件独立提交"。 |
| [轻微] 问题6：T5与T28之间的隐藏依赖未被识别——T5修复可能改变测试架构，T28需在T5落地后的新测试框架上重新设计 | 已修正：子任务C备注补充说明"T5完成后验证测试基础设施稳定再实施T28"。 |
