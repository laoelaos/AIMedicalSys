...
> 注：T13 根因同为 OOD 文本内在矛盾（见 T13 正文分析），双归属分类已在编码缺陷汇总表中呈现。
...
| T13 | SlidingWindowCounter.java | OOD 文本模糊 → 编码跟随其中一种解读（次因：编码未质疑矛盾） ↗ 主因为OOD文本模糊性
...
### P2（代码质量、测试完备性）
- T3、T4、T8、T10、T14、T16、T20、T21、T23-T34
...
2. **修复批 2：OOD 文档对齐**（T8、T17）
   ...
   - T17：在 OOD 10.3 节显式补充 AuthenticationEntryPoint/AccessDeniedHandler 路径 → 抽取 `MessageInterpolator` 组件，统一注入 GlobalExceptionHandler + RestAuthenticationEntryPoint + RestAccessDeniedHandler
   - 建议：先更新 OOD 文档（同一次提交），再更新编码（另一次提交）
   - **受影响存量测试**：
     - T8 → 无行为变更，仅 OOD 文档更正示例值，不影响现有测试。
     - T17 → `GlobalExceptionHandlerTest.java`：`MessageInterpolator` 抽取后，原有 `formatMessage` 内联测试可能需要调整为对 `MessageInterpolator` 的单独测试。`RestAuthenticationEntryPointTest` / `RestAccessDeniedHandlerTest`（若有）：需增加插值参数化场景以验证新注入路径的正确性。现有行为路径不变（UNAUTHORIZED/FORBIDDEN 等无动态参数），因此存量断言不失效。
...
6. **修复批 6：测试增强与文档对齐**（T3-T7、T20-T34）
   - T22 为误报，无需修复。
   ...
   |--------|------|--------|---------|--------|---------------|------|
   | A | 文档对齐 | T3 | 审查范围描述文件 | 1 | 独立，可最先执行 | 将 application/ 路径修正为模块实际路径 |
   | B | 新增测试 + 修正测试 | T6 + T21 | `AuthServiceTest.java` | 2 | ① T21（独立，可先行）→ ② T6（检测条件验证可先行，响应行为验证需待 T2 实施后更新） | T6 与 T21 同文件，合并减少冲突。T6 涉及 T2 行为变更依赖 |
   | C | 修正测试 + 新增测试 | T5 + T28 | `UserFacadeImplTest.java` | 2 | ① T5（解除 mock Converter）→ ② T28（新增 Repository 异常场景） | T5 修正测试设计，T28 增量新增，合并同文件减少 PR 数量 |
