...
| `AiService` | interface | AI 能力统一门面接口，定义调用的生命周期契约 |
...
#### `AiService` — AI 能力统一门面（interface）

**职责**：定义 AI 调用的生命周期契约：`invoke(input)` → `AiResult`，涵盖成功、降级、超时、不可用等状态。

**协作**：
- 业务模块通过 `AiService` 接口调用 AI 能力，不依赖具体实现
- `MockAiService` 在 Phase 0 实现该接口，返回占位/Mock 数据
- 真实 AI 在后续阶段实现该接口
...
**协作**：实现 `AiService` 接口，根据 AI 能力类型分发到对应的 Mock 数据生成逻辑。
...
业务模块调用 AI:
  AiService.invoke(request)
    ...
    └── FallbackAiService(invoke 失败) → DegradationStrategy.fallback(request)
...
| AI 能力接口形态 | 独立接口 vs 统一门面 | 统一门面 `AiService` | 13 项 AI 能力虽有不同输入/输出，但调用生命周期一致（调用→结果→降级），统一门面可复用降级、超时、熔断等横切逻辑 |
...
### 8.2 AI 能力接口清单（Phase 0 Mock 占位）

Phase 0 定义以下接口契约，所有接口返回 Mock 占位数据：

| 接口标识 | 对应 AI 能力 | 输入 | 输出 |
|---------|------------|------|------|
| `AiTriageService` | 3.4.1 智能分诊 | 患者主诉 | 推荐科室、医生列表 |
| `AiPrescriptionCheckService` | 3.4.2 AI 处方审核 | 处方药品列表 + 患者信息 | 审核结果、风险等级 |
| `AiMedicalRecordService` | 3.4.3 AI 病历生成 | 医患对话文本 | 结构化病历字段 |
| `AiDiagnosisService` | 3.4.4 AI 智能诊断 | 病情文本 | 诊断结论、置信度 |
| `AiReportService(检查)` | 3.4.5 AI 智能检查报告 | 检查原始数据 | 检查报告草稿 |
| `AiReportService(检验)` | 3.4.6 AI 智能检验报告 | 检验原始数据 | 检验报告草稿 |
| `AiImageAnalysisService` | 3.4.7 AI 影像分析 | 影像数据 | 识别结果 |
| `AiKnowledgeBaseService` | 3.4.8 AI 知识库问答 | 问题文本 | 回答文本 |
| `AiOrderExaminationService` | 3.4.9 AI 开立检查/检验 | 病情文本 | 项目推荐列表 |
| `AiPrescriptionAssistService` | 3.4.10 AI 辅助开方 | 诊断 + 检查结果 | 处方草案 |
| `AiExecutionOrderService` | 3.4.11 AI 执行顺序推荐 | 待执行任务列表 | 排序建议 |
| `AiSchedulingService` | 3.4.12 AI 医生排班 | 排班约束 | 排班方案 |
| `AiDiscussionConclusionService` | 3.4.13 AI 综合讨论结论 | 讨论记录 | 结论摘要 |

Phase 0 的 Mock 实现为每个接口返回固定结构占位数据，后续 Phase 替换为真实 AI 推理实现。接口清单未来可按 roadmap 逐步冻结细化。
