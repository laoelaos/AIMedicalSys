...
> **依赖方向说明**：业务模块（patient/doctor/admin）的实际依赖集为 `common` + `common-module-api` + `ai-api`，图中以 common-module-api 作为业务模块的主要依赖锚点。common-module-impl 和 ai-impl 仅由 application 模块直接引入，对业务模块完全不可见，Maven 编译期强制隔离。
...
**Phase 0 策略**：Phase 0 作为最小化骨架阶段，无登录页面、无认证 Controller、无 token 签发机制。因此 Phase 0 的 SecurityConfig 统一使用 `permitAll` 放通所有接口，确保后端可独立启动并通过健康检查。采用 `@Profile("phase0")` 条件化配置，与 Phase 1 的真实认证 SecurityConfig 通过 profile 自动切换，无需手动修改代码：
...
- 后端：`mvn spring-boot:run -pl application -am`（`-am` 即 `--also-make`，自动编译依赖模块并安装到本地仓库，避免首次构建时因依赖模块未安装导致解析失败）
...
DESIGN_WRITTEN:C:\Develop\Software\AIMedicalSys\Harness\redeliberations\202606151714_Phase0_OOD\a_v15_design_v1.md
