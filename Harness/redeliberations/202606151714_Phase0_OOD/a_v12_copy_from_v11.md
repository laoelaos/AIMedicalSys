...
modules/common-module/common-module-api (依赖 common, 仅含接口 + DTO)
  ↑
modules/common-module/common-module-impl (依赖 common-module-api, 含实体与 Service 实现)
  ↑  ↑  ↑
modules/patient  modules/doctor  modules/admin
  ...
  └──────────────┼──────────────┘
          ...
          modules/ai/ai-impl (依赖 ai-api, 仅由 application 引入)
                 ↑
            application (依赖所有 business 模块及 ai-impl 与 common-module-impl, 负责聚合启动)
```
...
**Common 模块依赖传播决策**：common 模块以 compile scope 声明了三个 Starter 依赖（spring-boot-starter-web、spring-boot-starter-security、spring-boot-starter-data-jpa），所有业务模块引入 common 后将无条件获得全部 transitive 依赖。逐项评估如下：
  ...
  - `spring-boot-starter-security`（compile，标注 `<optional>true</optional>`）：SecurityConfig 定义于 common 但仅被含 Controller 的模块需要；API 契约子模块（common-module-api、ai-api）无需安全上下文。标记 optional 后纯契约模块不会引入 security 的 transitive 依赖。需要 security 的模块（patient、doctor、admin、application）在自己的 POM 中显式声明该依赖（版本由父 POM 统一管理）
...
- `PageQuery`：包含 `page`（0-based，与 Spring Data JPA 默认一致；前端分页组件提交时需将 1-based 页码转换为 0-based）、`size`（默认值 20，上限 500，标注 `@Max(500)` 防止恶意大分页导致 OOM；需结合 `@Valid` 使校验生效）、`sort`（`String` 类型，格式为 `"fieldName,direction"`，direction 为 `asc` 或 `desc`，如 `"createdAt,desc"`）。所有分页 Controller 参数需标注 `@Valid` 以触发校验注解。`@Valid` 对 `@ModelAttribute`（GET 查询参数绑定）和 `@RequestBody`（POST JSON 绑定）均生效，前提是 `spring-boot-starter-validation` 在类路径上（父 POM 的 `<dependencyManagement>` 中已统一声明，含 Controller 的业务模块以 `compile` scope 引入即可）。
...
- `NoOpDegradationStrategy`（class，`ai-impl`）：Phase 0 默认注册的降级策略，`shouldDegrade()` 始终返回 `false`（永不主动降级），`fallback()` 返回 `null`。标注 `@ConditionalOnMissingBean(DegradationStrategy.class)`，确保 Phase 2+ 被真实策略替换时自动退让
- `TimeoutDegradationStrategy`（class，`ai-impl`）：基于超时阈值的降级策略，归 Phase 2+ 实现，Phase 0 不注册
- `FallbackAiService`（class，`ai-impl`）：包装降级逻辑的 `AiService` 装饰器，在 AI 调用失败时按策略执行降级。内部通过构造器注入 `List<DegradationStrategy>` 策略列表（Phase 0 仅含 NoOpDegradationStrategy，Phase 2+ 可传入多个策略）；遍历策略列表，任一策略 `shouldDegrade()` 返回 `true` 即触发降级。内部同时通过 `@Autowired` + `@Lazy` + `ObjectProvider<AiService>` 延迟获取当前可用的底层 AiService 实现（MockAiService 或真实实现），不依赖硬编码 Bean 名称。降级触发时构造 `DegradationContext` 实例并传递给 `DegradationStrategy.shouldDegrade()`。**兜底保护**：当 `ObjectProvider.getIfAvailable()` 返回 null（无可用的 AiService 实现时），FallbackAiService 不抛出异常，直接返回 `AiResult` 实例（`success=false`、`degraded=true`、`data=null`），确保业务链路不因 AI 模块缺失而断裂。Phase 0 的构造器签名为：
  ```
  public FallbackAiService(ObjectProvider<AiService> aiServiceProvider,
...
第四阶段（集成测试）: mvn verify -pl integration -Dsurefire.skip=true（运行集成测试，含 Failsafe 插件；-Dsurefire.skip=true 仅跳过 Surefire 单元测试，不影响 Failsafe 集成测试执行；-DskipTests 也会影响 Failsafe 故避免使用）
第五阶段（测试 + 前端）: mvn test → 构建前端（根目录 npm ci → 各 apps/* 中 npm run build）→ 归档制品。根 `package.json` 中定义 `build:all` 脚本（`"build:all": "npm run build --workspaces"`），各 apps/* 的 `package.json` 定义各自 `build` 脚本，CI 执行 `npm run build:all` 统一构建三端应用
```
...
DESIGN_WRITTEN:C:\Develop\Software\AIMedicalSys\Harness\redeliberations\202606151714_Phase0_OOD\a_v12_copy_from_v11.md
