...
- `PageQuery`：包含 `page`（0-based，与 Spring Data JPA 默认一致；前端分页组件提交时需将 1-based 页码转换为 0-based）、`size`（默认值 20，上限 500，标注 `@Max(500)` 防止恶意大分页导致 OOM；需结合 `@Valid` 使校验生效）、`sort` 字段。所有分页 Controller 参数需标注 `@Valid` 以触发校验注解。
...
// 编译期耦合及序列化循环引用。UserDTO 定义在 common-module 的 dto 子包
// （com.aimedical.modules.commonmodule.dto）中，仅包含 userId、userName、userType 等必要字段。
...
DESIGN_WRITTEN:C:\Develop\Software\AIMedicalSys\Harness\redeliberations\202606151714_Phase0_OOD\a_v10_copy_from_v9.md
