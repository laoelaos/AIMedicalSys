...
- `FallbackAiService`：作为装饰器始终注册为 Bean，通过 `@Primary` 确保其被注入到业务模块；内部持有真实或 Mock 的实现 Bean
...
- `DegradationStrategy`（interface，`ai-api`）：定义降级判定逻辑 `boolean shouldDegrade()` 和降级行为 `<T, R> R fallback(T input)`。泛型参数 `T` 为输入类型，`R` 为降级返回值类型，与 `AiResult<T>` 的解包类型一致
- `TimeoutDegradationStrategy`（class，`ai-impl`）：基于超时阈值的降级策略
- `FallbackAiService`（class，`ai-impl`）：包装降级逻辑的 `AiService` 装饰器，在 AI 调用失败时按策略执行降级
...
Phase 0 CI 流水线按模块依赖顺序分阶段构建：

```
第一阶段（基础层）: checkout → mvn compile -pl common,modules/common-module,modules/ai/ai-api
第二阶段（业务层）: mvn compile -pl modules/patient,modules/doctor,modules/admin,modules/ai/ai-impl
第三阶段（聚合层）: mvn compile -pl application
第四阶段（测试 + 前端）: mvn test → 构建前端（各 apps/* 中 npm run build）→ 归档制品
```

- 第一阶段构建零依赖的基础模块，确保共享契约先就绪
- 第二阶段构建依赖基础层的业务模块
- 第三阶段构建依赖所有业务模块的启动聚合层
- 第四阶段运行单元测试并验证前端构建
