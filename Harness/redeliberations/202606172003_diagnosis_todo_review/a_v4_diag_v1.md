...
此方案的联动影响：`apiGet`/`apiPost` 返回类型从 `Promise<ApiResponse<T>>` 调整为 `Promise<T>`；error 拦截器需新增业务错误码分支及区分逻辑。
...
无论采用哪种方案，`apiGet`/`apiPost` 等包装函数的返回类型均需从 `Promise<ApiResponse<T>>` 调整为 `Promise<T>`，因为拆包后去掉了外层 `Result` 包装。
...
**推荐理由**：方案 B 改动量最小（仅修改 3 处：构造器加 3 行、handleEmptyDelegates 移除 IF/ELSE、删字段）；与 OOD §3.4 原意完全一致（构造器 ERROR + 后续 WARN）；构造器执行在 Spring 单线程启动期，无需 AtomicBoolean 等线程安全机制。
