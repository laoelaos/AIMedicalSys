...
- 建议在类型定义文件中定义 `BusinessError = { code: string; message: string }`（或 discriminated union）并在消费侧通过类型守卫判断
...
5. **修复后验证闭环**：
   ...
   - 确认 logback 配置（`logback.xml`、`logback-spring.xml` 或 `spring-default.xml`）中未对 `com.aimedical.modules.ai.impl.fallback.FallbackAiService` 类或该包设置过滤规则阻止 ERROR 级别日志输出。若存在全局根级别限制（如 `<root level="WARN">`），则构造器输出的 ERROR 在生产环境中将不可见，需单独为该包开放 ERROR 级别（如 `<logger name="com.aimedical.modules.ai.impl.fallback" level="ERROR"/>`）

**推荐理由**：方案 B 改动量小（修改 3 处源代码 + 1 处测试 + 1 处验证）；与 OOD §3.4 原意完全一致（构造器 ERROR + 后续 WARN）；构造器执行在 Spring 单线程启动期（基于当前 Spring 单线程 Bean 创建的实现行为，非 API 契约保证，但在 Phase 0 框架版本下实际风险极低），无需 AtomicBoolean 等线程安全机制。
