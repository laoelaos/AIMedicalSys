...
- **T10 → 真实存在**：代码未实现 OOD §3.4 的"构造器阶段 ERROR"要求。OOD 要求在 Spring DI 环境下技术可行（构造器执行时 `List<AiService>` 已可用），并非不切实际。当前代码采用的惰性检测模式将 ERROR 从启动期延迟到首次调用期，是代码实现与 OOD 规范之间的偏差，而非 OOD 文档问题。详见 T10 分析。
...
(error) => {
  ...
  if (error.response === undefined) {
    return Promise.resolve({ code: 'NETWORK_ERROR', message: '网络不可达，请检查网络连接' });
  }
  // ③ HTTP 错误（有具体响应状态码）
  if (error.response.status === 401) { ... }
  if (error.response.status === 403) { ... }
  return Promise.resolve({ code: 'HTTP_ERROR', message: `请求失败（${error.response.status}）` });
}
```
...
此方案的联动影响：`apiGet`/`apiPost`/`apiPut`/`apiDelete` 返回类型从 `Promise<ApiResponse<T>>` 调整为 `Promise<T>`。**零冲击面**——经代码库实测，前端除测试文件 `packages/shared/src/api/__tests__/interceptors.test.ts` 外，不存在任何对上述四个包装函数的现有调用，真实应用代码（`packages/ui-core/`、`packages/*/src/` 不含 `__tests__/` 目录）无一使用。返回类型变更仅需同步更新上述测试文件中的类型断言和集成测试预期，无需改动其他前端代码；error 拦截器需新增业务错误码分支及区分逻辑。

B. **success 拦截器内直接处理**：在 success 拦截器内检查 `code !== "SUCCESS"` 并调用统一错误处理函数（如消息弹窗），不经过 error 拦截器。此方式避免了 error 拦截器区分逻辑的复杂度，与 OOD §4.2 的"走统一错误处理"语义差异可通过错误处理函数抽象来弥合。
...
无论采用哪种方案，`apiGet`/`apiPost`/`apiPut`/`apiDelete` 等包装函数的返回类型均需从 `Promise<ApiResponse<T>>` 调整为 `Promise<T>`，因为拆包后去掉了外层 `Result` 包装。
...
**Spring DI 环境可行性分析**：在 Spring 中，构造器在 Bean 创建阶段（启动期）执行，此时注入的 `List<AiService>` 已可用。在构造器中直接检查 `delegates.isEmpty()` 并输出 ERROR 日志是技术可行的，OOD 的构造器阶段要求并非不切实际。因此，OOD §3.4 的规范本身是合理的，根因在代码实现未对齐规范，而非 OOD 文档存在工程约束遗漏。
...
4. **同步更新测试文件** `FallbackAiServiceTest.java`：`shouldLogErrorOnFirstCallThenWarnOnSubsequent()`（第 117-142 行）需重构以验证新行为——构造器阶段输出 ERROR、首次调用仅输出 WARN。当前测试创建 `FallbackAiService` 后立即附加 ListAppender（第 119-124 行），因此构造器输出的 ERROR 会被捕获。修复后执行流程变为：第 124 行构造时产生 1 条 ERROR → 第 126 行首次 triage 产生 1 条 WARN → 第 127 行 `appender.list.size()` 从期望 1 变为 2，断言需同步调整。建议重构为两个独立测试方法：`shouldLogErrorOnConstruction()` 验证构造器阶段输出 ERROR，`shouldLogWarnOnSubsequentCalls()` 验证首次及后续调用仅输出 WARN，并移除第 130 行的 `⚠️` 待办注释标记。

**推荐理由**：方案 B 改动量小（修改 3 处源代码 + 1 处测​​试）；与 OOD §3.4 原意完全一致（构造器 ERROR + 后续 WARN）；构造器执行在 Spring 单线程启动期（基于当前 Spring 单线程 Bean 创建的实现行为，非 API 契约保证，但在 Phase 0 框架版本下实际风险极低），无需 AtomicBoolean 等线程安全机制。

方案 A（备选——在构造器调用 `handleEmptyDelegates()` 同时修改方法体移除 AtomicBoolean）与方案 B 效果等价，但方案 B 将 ERROR 日志直接写入构造器使代码意图更清晰——构造器负责启动期严重问题检测，`handleEmptyDelegates()` 仅负责运行期日志，职责分离更干净。
