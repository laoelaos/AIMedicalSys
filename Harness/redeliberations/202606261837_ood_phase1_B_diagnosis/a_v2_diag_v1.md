...
| P0 | T23 | 真实缺陷 | 实现编码偏差 | MenuServiceImpl | - |
...
| P2 | T4 | 真实缺陷 | 实现编码偏差 | GlobalErrorCode | T11（消息变更影响字符串匹配）、T27（同类消息偏差） |
| P2 | T5 | 真实缺陷 | 实现编码偏差 | AuthServiceImpl | - |
| P2 | T9 | 真实缺陷 | 实现编码偏差 | UserConverter / UserFacadeImpl | - |
| P2 | T10 | 真实缺陷 | 实现编码偏差 | JwtConfig | T1（同为JWT启动验证） |
| P2 | T11 | 真实缺陷 | 实现编码偏差 | RestAuthenticationEntryPoint | T4/T27（消息变更使匹配静默失效） |
...
**统计：** 26/26 项确认为真实缺陷（0 项误报）。其中 21 项根因归属于实现编码偏差，1 项归属于设计与实现约定不一致（T14），4 项归属于测试覆盖不完整（T15/T16/T17/T18）。
...
- **风险与注意事项：**
  - 与 T11 联动：`RestAuthenticationEntryPoint` 使用 `authException.getMessage().contains(GlobalErrorCode.ACCOUNT_DISABLED.getMessage())` 判断账户禁用，但不受 UNAUTHORIZED 消息变更影响（它检查的是 ACCOUNT_DISABLED 消息）。仍需关注 T11 本身的修复。
...
- **修改建议：**
  - **前置调研：** 安全审计日志应满足可审计性（不可篡改、可追溯、包含操作人/时间/结果）。`log.info()` 不满足审计要求（日志等级可被运行时调整、不强制持久化、无结构化审计字段）。需先确认项目中是否已有的审计框架（如 Spring Audit Log、自定义 AuditLogger），再决定实现方案：
    - **方案 A：** 若项目已有 AuditLogger，复用其记录登出审计事件（含 refreshToken 掩码、操作人 userId、时间戳、操作结果）。
    - **方案 B：** 若无现成审计框架，新增 `SecurityAuditLogger` 组件，提供 `logAudit(String action, String operator, String target, boolean success)` 方法，将审计日志写入独立日志文件或数据库审计表。
  - 代码位置：
...
- **风险与注意事项：**
  - JwtConfig.validate() 执行时间在 JwtTokenProvider.init() 之前（均为 @PostConstruct，但 JwtConfig 是 @ConfigurationProperties，先于 @Component 初始化）。T1 修复时应注意 JwtTokenProvider.init() 中的校验不重复 JwtConfig 的校验职责。
...
- **风险与注意事项：**
  - 虚拟哈希值应使用一个实际生成的 BCrypt 哈希，确保 matches() 不会因格式错误而抛出异常。建议在测试中生成一个有效哈希并硬编码为常量。
  - 与 T3 的关系：无直接依赖。但两者均在 AuthServiceImpl.login() 中修改代码，建议在一次提交中完成以避免合并冲突。

### T14: SlidingWindowCounter 中 ReentrantLock 作用域不完整（P2）
...
- **风险与注意事项：**
  - 新增字段后需添加 getter/setter 方法。`PermissionFunction` 当前未使用 Lombok，需手动添加。`component` 不是 Java 关键字，列名在 Hibernate 中默认映射为字段名 `component`，与表列名一致则无需 `@Column(name = "component")`。但 `component` 是 Hibernate/JPA 的关键概念，建议显式指定 `@Column(name = "component")` 以避免歧义。

### T23: getUserMenuTree 未使用 @EntityGraph，存在 N+1 查询风险（P0）
...
- **修改建议：**
  ...
  - 若确认数据量级很小（用户岗位数 ≤3 且每个岗位功能数 ≤20），可在 EntityGraph 中仅保留 `posts` 属性路径，在风险与注意事项中注明此决策依据并标注为已知剩余优化空间。
