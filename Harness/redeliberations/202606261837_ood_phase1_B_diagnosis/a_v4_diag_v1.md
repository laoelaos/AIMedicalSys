...
| **批次 3：JWT 认证重构** | T2, T12, T1, T10 | T2↔T12 必须同步提交（同一文件的注入切换 + userId 提取归一化）；T1 和 T10 在该文件修改后独立修复。注意 T2 将 `JwtUtil` 替换为 `JwtTokenProvider` 后，SecurityConfigPhase1 的 Bean 组装方法签名需同步调整 |
...
| **批次 5：Controller 层修复** | T6, T7, T19, T21 | 四个 Controller 端点修复互无依赖，可并行。注意 T6（路径变更）和 T19（HTTP 方法变更）涉及 API 契约变化，需前端同步配合 |
...
| **批次 7：测试补充** | T15, T16, T17, T18 | 在以上功能修复完成后执行，确保断言与修复后行为对齐。T16 与 T3 强依赖——需 T3 修复后再确认消息插值结果。 |

**批次间执行依赖：** 批次 1 → 批次 2-5（可并行）→ 批次 6 → 批次 7。批次 2-5 彼此无依赖，可并行执行以缩短修复周期。
...
- **修改建议：**
  - **前置调研确认方法：** 安全审计日志应满足可审计性（不可篡改、可追溯、包含操作人/时间/结果）。`log.info()` 不满足审计要求（日志等级可被运行时调整、不强制持久化、无结构化审计字段）。快速确认项目中是否已有审计框架，可按以下顺序执行：
    1. 在 `AIMedical/backend/` 下搜索关键词 `AuditLogger`、`AuditLog`、`AuditRecord`、`audit` 相关类名——使用 IDE 全局搜索或 `grep -r "class.*Audit" --include="*.java"`）
    2. 检查 `common-module` 或 `framework` 模块中是否有 `AuditAspect`、`@AuditLog` 注解或 `AuditEventPublisher`
    3. 若以上均未命中，则项目中无现成审计框架，直接选用方案 B
    - **方案 A：** 若项目已有 AuditLogger，复用其记录登出审计事件（含 refreshToken 掩码、操作人 userId、时间戳、操作结果）。
    - **方案 B：** 若无现成审计框架，新增 `SecurityAuditLogger` 组件，提供 `logAudit(String action, String operator, String target, boolean success)` 方法，将审计日志写入独立日志文件或数据库审计表。方案 B 可作为默认选择，即使项目后续发现已有框架，`SecurityAuditLogger` 的适配成本也很低（在实现上包装为委托调用即可）。
  - 代码位置：
