...
该文档整体骨架思路清晰，契约先行、模块隔离的核心方向正确。但经逐节推演，当前存在 **2 个阻断性问题（P1）**、**9 个主要问题（P2）** 和 **3 个次要问题（P3）**，不建议在未修订时直接进入编码阶段。
...
| **现象** | §3.2 第 494 行定义 `@SQLDelete("UPDATE entity SET deleted = true WHERE id = ?")` 在 BaseEntity 上。使用了字面量 `entity` 而非 Hibernate 6 提供的 `{h-table}` 占位符，Hibernate 不会将其自动解析为子类对应的实际表名。 |
| **根因** | `@SQLDelete` 注解内为编译期常量字符串，虽然 Hibernate 6 支持 `{h-table}` 占位符来动态解析子类表名，但原文使用了字面量 `entity` 而非 `{h-table}`，导致生成的 SQL 为 `UPDATE entity SET deleted = true WHERE id = ?` 而非 `UPDATE user SET deleted = true WHERE id = ?`。 |
| **因果链** | 文档期望"所有实体继承 BaseEntity 即获得软删除能力"→ `@SQLDelete` 中用了字面量 `entity` 而非 `{h-table}` → 运行时生成的 SQL 为 `UPDATE entity SET ...` 而非 `UPDATE user SET ...` → SQL 语法错误或更新错表 → 软删除完全不可用 |
| **影响范围** | 所有继承 BaseEntity 的实体（User、Role、Post、Function 及未来业务实体）的删除操作。 |
| **修复方向** | 将 `@SQLDelete("UPDATE {h-table} SET deleted = true WHERE id = ?")` 使用 `{h-table}` 占位符替代字面量 `entity`。若需进一步避免继承层级下的歧义，可将 `@SQLDelete` + `@SQLRestriction` 从 BaseEntity 移除，改为各实体自行配置。 |

---

## 二、主要问题（P2 — 影响设计一致性和可编码性）

### P2-1. POM XML 骨架片段与正文描述的依赖声明不一致
...
| **根因** | 文档正文明确陈述了依赖的存在，但附带的 XML 骨架片段中遗漏了这些声明。父 POM 片段标注为"基础结构骨架"（§2.1 第 121 行），属于示意性代码而非完整 POM 规格。两者不一致会导致实现者不确定"骨架"与"完整规格"之间的差距。 |
| **因果链** | 实现者按正文要求在代码中添加注解 → 但 POM 骨架中缺少相应依赖 → 需自行确认版本并补全 → 不同开发者可能使用不同的版本号或遗漏 scope 配置 |
| **影响范围** | springdoc-openapi：所有含 Controller 的模块（patient/doctor/admin）。H2：application 模块的启动阶段。 |
| **修复方向** | 统一正文表述与 XML 骨架：在父 POM 骨架的 `<dependencyManagement>` 中补全 `springdoc-openapi-starter-webmvc-ui` 和 `com.h2database:h2` 的声明（或添加注释说明"此处省略了关键依赖，完整列表见正文 §8.3 及 §9.1"）。确保实现者拿到骨架 POM 即可编译通过。 |

---

### P2-2. AI 装配策略泄漏实现细节到业务模块
...
### P2-3. 中间层聚合 POM 缺少用途说明

| 属性 | 内容 |
|------|------|
| **位置** | §2.1 第 210-256 行（中间层聚合 POM 结构） |
| **现象** | 根 POM 的 `<module>` 列表中按扁平结构列出了所有叶子模块（`modules/common-module/common-module-api`、`modules/common-module/common-module-impl`、`modules/ai/ai-api`、`modules/ai/ai-impl`）。同时定义了 `modules/common-module/pom.xml` 和 `modules/ai/pom.xml` 两个中间层聚合 POM，各自声明其子模块。但根 POM 从未引用中间聚合 POM。 |
| **根因** | 这是 Maven 的标准"部分聚合"（partial aggregation）模式：根 POM 扁平列出所有叶子模块以支持 CI 流水线的精确 `-pl` 路径；中间聚合 POM 为子模块提供共享 parent POM 引用点和独立构建入口（通过 `mvn -pl modules/common-module -am` 触发）。两者完全可共存，非互斥方案。文档缺少注释说明中间聚合 POM 的双重用途，可能使不熟悉此模式的开发者误以为存在冲突。 |
| **因果链** | 实现者看到两套并行结构 → 不确定是否冲突 → 若误删中间聚合 POM 则子模块的 `<parent>` 链接失效；若误改根 POM 引用中间聚合 POM 则 CI 的 `-pl` 路径需全部调整 |
| **影响范围** | Maven 构建配置维护者、新加入团队对项目结构的理解。 |
| **修复方向** | 在中间聚合 POM 的 `<name>` 或 XML 注释中说明其用途，如："${artifactId} — 聚合父 POM，为子模块提供共享 parent 引用和独立构建入口。根 POM 以扁平结构直接引用叶子模块，两者并行不悖"。 |

---

### P2-4. common-module-impl 的 Phase 0 范围前后矛盾
...
### P2-6. SecurityConfig 的 Phase 0 → Phase 1 切换方案不完善
...
### P2-7. ErrorCode interface 的方法契约未明确定义
...
### P2-8. `sort` 参数设计为自定义 String 格式，与 Spring Data 原生机制不兼容
...
| P2 | POM XML 骨架与正文依赖声明不一致 | §2.1 / §8.3 / §9.1 | 文档不一致 |
| P2 | AI 装配泄漏 @Qualifier 到业务模块 | §3.4 / §7 | 接口隔离违反 |
| P2 | 中间聚合 POM 缺少用途说明 | §2.1 | 文档完整性 |
...
当前 `Docs/04_ood_phase0.md` 的 **2 个 P1 问题必须全部修订**方可进入编码阶段。P2 问题中的 P2-2（AI 装配泄漏）在 P1 修复时建议一并解决，其余 P2/P3 可在 Phase 1 编码前修订。

最优先处理的三项：
1. 固定 AiService 契约（P1-1）—— 影响所有业务模块的 AI 调用代码
2. 修复 BaseEntity @SQLDelete 使用 `{h-table}` 占位符（P1-2）—— 所有实体软删除不可用
3. 统一 POM 骨架与正文的依赖声明（P2-1）—— 确保骨架可编译通过且与正文一致
...
| 质询意见 | 回应 |
|---------|------|
| P1-4（业务模块缺少 API/impl 分层）— 误诊。patient/doctor/admin 为 REST 对外暴露的终端模块，与 common-module 和 ai（共享服务模块）架构定位不同，无需 api/impl 拆分。 | **接受。** 已从诊断报告中移除 P1-4。patient/doctor/admin 作为终端模块，其 Controller 即为对外 API 层，flatten 结构是合理设计。跨终端模块调用在 Phase 1+ 通过 REST API 或事件机制实现，无需 Java 接口门面子模块。 |
| P2-2（Maven 聚合结构两套不可兼得）— 误判。根 POM 扁平列出叶子模块与中间聚合 POM 可共存（Maven 标准部分聚合模式），中间聚合 POM 非死代码。 | **接受。** 已修订 P2-2 内容，从"两套不可兼得的 reactor 组织"改为"中间聚合 POM 缺少用途说明"。当前并行结构为有效 Maven 实践，仅需补充注释说明中间聚合 POM 的双重用途。 |
| P1-2（BaseEntity @SQLDelete）— 结论正确但根因有技术误差。Hibernate 6 实际提供 `{h-table}` 占位符，真正问题是使用了字面量 `entity` 而非 `{h-table}`。 | **接受。** 已修订 P1-2 根因描述：明确 Hibernate 6 支持 `{h-table}` 占位符，根本问题是使用了字面量 `entity`。修复方向相应修正为使用 `{h-table}`。 |
| P1-3（POM 遗漏依赖）— 严重程度偏高。父 POM XML 片段为"基础结构骨架"，属示意性代码。正文已描述依赖应存在，更宜作为 P2。 | **接受。** 已将该问题从 P1 降级为 P2（现为 P2-1），标题从"POM 中遗漏关键依赖声明，直接导致构建失败"改为"POM XML 骨架片段与正文描述的依赖声明不一致"。根因描述中补充说明了骨架代码的示意性定位与正文之间的不一致性。 |
| 其余质询意见（P1-1、P2-1、P2-3~P2-8、P3-1~P3-3）—— 均判定为[通过]。 | **确认通过。** 对应问题维持原有诊断结论不变，无需修订。 |
