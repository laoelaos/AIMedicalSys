...
修复 Phase1B 报告中的 3 个 P2 问题：T12（JwtTokenProvider URL-safe Base64 校验）、T14（JwtUtil.generateToken 遗留 claims 清理并添加 jti）、T16（SimpleMessageInterpolator 命名占位符优化）。修改 3 个独立源文件及对应的测试文件，无交叉依赖。
...
| `modules/common-module/common-module-impl/src/test/java/.../auth/jwt/JwtTokenProviderTest.java` | MODIFY | T12：测试数据改为含 `!` 的字符串，断言消息改为 "URL-safe" |
...
| `modules/common-module/common-module-impl/src/test/java/.../jwt/JwtUtilTest.java` | MODIFY | T14：移除 shouldGenerateTokenWithPosition 用例，更新 shouldParseValidToken 和 shouldGetRoleFromValidToken 断言 |
...
| 用例 | 操作 | 原因 |
|------|------|------|
| `shouldGenerateTokenWithPosition` (GenerateTokenTests, line 59-65) | **移除** | position claim 不再写入，该用例已验证的行为不复存在 |
| `shouldParseValidToken` (ParseTokenTests, line 81) | **修改** | 移除 `assertEquals("ADMIN", claims.get("role"))` — role claim 不再写入 generateToken；保留 userId 和 username 断言 |
| `shouldGetRoleFromValidToken` (GetRoleTests, line 179-185) | **修改** | 改为验证新生成 token 的 `getRole()` 返回 null（role claim 不再写入） |
...
- `shouldReplaceNumberedPlaceholders` — 数字占位符（含 {0}）→ 预检匹配 → MessageFormat 正常路径
- `shouldReplaceNamedPlaceholdersByPosition` — 命名占位符（不含数字）→ 预检不匹配 → replaceFirst 路径（行为不变）
- `shouldReuseSameArgForMultiplePlaceholders` — 数字占位符（{0} 重复）→ MessageFormat 正常路径
- `shouldReturnTemplateForNoPlaceholdersWithArgs` — 无占位符（不含 `{`）→ 预检不匹配 → replaceFirst 路径（无替换）
