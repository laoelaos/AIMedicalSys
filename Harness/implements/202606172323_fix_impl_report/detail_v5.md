...
- `ApiError` 在结构上是 `BusinessError` 的子类型（narrow union → `string`），因此 `Promise<T | BusinessError>` 可同时容纳 error interceptr 的 `ApiError` 对象和 success interceptor 返回的业务错误对象
...
| 审查意见 | 修改措施 |
|---------|---------|
| verify FAILED：2 个 non-SUCCESS 测试因 handler 返回 Promise 而非 BusinessError 对象导致测试失败 | success 拦截器 non-SUCCESS 分支移除 `Promise.resolve()` 包装，直接返回 BusinessError 对象，与 SUCCESS 分支 `return body.data` 的同步行为一致 |
